...
const { LORA_MODALITY_VIDEO, LORA_PROFILE_TYPE } = require("../config/lora");
const {
...
module.exports = (app, deps) => {
  ...
  app.post("/replicate/video/generate", deps.requireUserMiddleware, async (req, res) => {
    ...
    const modelKey = resolveModelDefault(req.body?.model, directorFallbackConfig.video.videoModel);
    ...
    if (modelConfig.requiresImage) {
      ...
      if (inputKey) {
        try {
          const { buffer, contentType } = await fetchS3ImageBuffer(mediaBucket, inputKey);
