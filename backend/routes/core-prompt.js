module.exports = (app, deps) => {
  ...
  app.get("/prompt-helper/options", async (req, res) => {
    ...
    try {
      ...
      const optionMap = new Map(items.map((item) => [item.key || "", item.options || []]));
      const getOption = (key) =>
        Array.isArray(optionMap.get(key)) ? optionMap.get(key) : promptHelperDefaults[key] || [];
      res.json({
    ...
    try {
      const command = new InvokeModelCommand({
        ...
        body: JSON.stringify({
          ...
          max_tokens: 300,
          temperature: 0.2,
          system: "You write concise, expressive positive prompts for AI image generation.",
          messages: [
            {
              role: "user",
      ...
      res.json({
        ...
        negativePrompt: [negativePrompt, singleCharacterNegative].filter(Boolean).join(", "),
        modelId: promptHelperModelId,
      });
    } catch (error) {
