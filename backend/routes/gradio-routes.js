module.exports = (app, deps) => {
  ...
  app.post("/gradio/image/generate", deps.requireUserMiddleware, async (req, res) => {
    ...
    const resolvedWidth = Number.isFinite(width) ? width : modelConfig.defaultWidth;
    const resolvedHeight = Number.isFinite(height) ? height : modelConfig.defaultHeight;
    ...
    const resolvedAspectRatio = modelConfig.aspectRatio || `${resolvedWidth} x ${resolvedHeight}`;
    ...
    const resolvedUseUpscaler =
      typeof modelConfig.useUpscaler === "boolean" ? modelConfig.useUpscaler : false;
    const resolvedUpscalerStrength = Number.isFinite(modelConfig.upscalerStrength)
      ? modelConfig.upscalerStrength
      : 0;
    const resolvedUpscaleBy = Number.isFinite(modelConfig.upscaleBy) ? modelConfig.upscaleBy : 1;
    const resolvedAddQualityTags =
      typeof modelConfig.addQualityTags === "boolean" ? modelConfig.addQualityTags : false;
    ...
    const resolvedNegativePrompt =
      negativePrompt || (resolvedUseNegativePrompt ? DEFAULT_GRADIO_NEGATIVE_PROMPT : "");
    const shouldRandomizeSeed =
      typeof randomizeSeed === "boolean" ? randomizeSeed : !Number.isFinite(Number(seed));

    try {
      const hfToken = process.env.HUGGING_FACE_TOKEN || process.env.HUGGINGFACE_TOKEN;
      const client = await getGradioSpaceClient(modelConfig.spaceId, hfToken);
      ...
      const resolvedImageOutput = Array.isArray(imageOutput) ? imageOutput[0] : imageOutput;
      const { buffer, contentType } = await resolveGradioImageBuffer(resolvedImageOutput);
