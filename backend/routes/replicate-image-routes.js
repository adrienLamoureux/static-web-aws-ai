...
const { LORA_MODALITY_IMAGE, LORA_PROFILE_TYPE } = require("../config/lora");
const {
...
module.exports = (app, deps) => {
  ...
  app.post("/replicate/image/generate", deps.requireUserMiddleware, async (req, res) => {
    ...
    const numImages = Math.min(Math.max(isDiffScheduler ? 2 : requestedImages, 1), 2);
    const seed = req.body?.seed;
    const characterId = normalizeString(req.body?.characterId);
    const seeds = isDiffScheduler
      ? Array.from({ length: numImages }, () =>
          Number.isFinite(Number(seed)) ? Number(seed) : Math.floor(Math.random() * 2147483647)
        )
    ...
    const trimmedNegativePrompt = negativePrompt ? clampPromptTokens(negativePrompt) : undefined;
    const promptWasTrimmed = trimmedPrompt.trim() !== prompt.trim();
    const negativeWasTrimmed =
      Boolean(negativePrompt) && trimmedNegativePrompt?.trim() !== negativePrompt.trim();
    const promptNotice = [
      promptWasTrimmed
    ...
    try {
      ...
      const loraProfileNotice = characterId ? `Character LoRA profile ${characterId} applied.` : "";

      if (modelConfig.usePredictions) {
        let prediction = null;
        const resolvedSeed = Number.isFinite(Number(seed)) ? Number(seed) : undefined;
