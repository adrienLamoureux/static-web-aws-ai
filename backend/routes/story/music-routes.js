...
module.exports = function registerStoryMusicRoutes(deps) {
  ...
  const { STORY_MUSIC_DEFAULT_INPUT } =
    buildStoryMusicDefaultInput(normalizePromptFragment);

  const persistStorySceneMusic = createPersistStorySceneMusic(deps);

  const router = Router();

  router.post("/sessions/:id/scenes/:sceneId/music", deps.requireUserMiddleware, async (req, res) => {
    ...
    try {
      ...
      if (prediction.status === "succeeded") {
        ...
        return res.json({
          ...
          musicKey,
          track: trackItem ? await mapMusicTrackResponse(normalizePromptFragment, s3Client, GetObjectCommand, getSignedUrl, bucket, trackItem) : null,
        });
      }
  ...
  router.get("/sessions/:id/scenes/:sceneId/music", deps.requireUserMiddleware, async (req, res) => {
    const userId = req.user?.sub;
    const sessionId = req.params.id;
    const sceneId = req.params.sceneId;
