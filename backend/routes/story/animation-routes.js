...
module.exports = function registerStoryAnimationRoutes(deps) {
  ...
  router.get("/sessions/:id/scenes/:sceneId/animation", deps.requireUserMiddleware, async (req, res) => {
    const userId = req.user?.sub;
    const sessionId = req.params.id;
    const sceneId = req.params.sceneId;
    ...
    try {
      ...
      if (!resolvedPredictionId) {
        const existingVideoUrl = await signSceneVideoUrl(s3Client, GetObjectCommand, getSignedUrl, bucket, sceneItem);
        return res.json({
          sceneId,
