...
module.exports = function registerStoryIllustrationRoutes(deps) {
  const {
    mediaTable,
    getItem,
    buildMediaPk,
  ...
  router.post("/sessions/:id/illustrations", deps.requireUserMiddleware, async (req, res) => {
    ...
    try {
      ...
      const sceneContextForPrompt = isOpening
        ? compactScenePayload({
