...
module.exports = function registerStoryMusicSelectionRoutes(deps) {
  ...
  router.post(
    "/sessions/:id/scenes/:sceneId/music/favorite",
    ...
    async (req, res) => {
      ...
      try {
        ...
        const libraryKey = buildStoryMusicLibraryKey(
          buildUserPrefix,
          userId,
          trackId,
          sourceExtension
  ...
  router.post(
    "/sessions/:id/scenes/:sceneId/music/recommend",
    deps.requireUserMiddleware,
    async (req, res) => {
      const userId = req.user?.sub;
      const sessionId = req.params.id;
  ...
  router.post(
    "/sessions/:id/scenes/:sceneId/music/select",
    deps.requireUserMiddleware,
    async (req, res) => {
      const userId = req.user?.sub;
      const sessionId = req.params.id;
