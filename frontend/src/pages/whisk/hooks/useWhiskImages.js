...
export const useWhiskImages = ({ apiBaseUrl, cacheKey, cacheMaxAge, onError }) => {
  ...
  useEffect(() => {
    ...
    refreshImages();
  }, [apiBaseUrl, cacheKey, cacheMaxAge, onError, refreshImages]);
  ...
  const toggleImageFavorite = useCallback(
    async (image) => {
      ...
      updateImages((prev) =>
        prev.map((item) => (item.key === image.key ? { ...item, favorite: nextFavorite } : item))
      );
      try {
        ...
        updateImages((prev) =>
          prev.map((item) =>
            item.key === image.key ? { ...item, favorite: Boolean(image.favorite) } : item
          )
        );
