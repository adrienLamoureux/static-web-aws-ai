    ...
    <div style={{ border: '1px solid var(--skr-border)', borderRadius: 8, overflow: 'hidden' }}>
      <button
        ...
        style={{
          ...
          background: 'var(--skr-surface)', border: 'none', cursor: 'pointer',
          display: 'flex', justifyContent: 'space-between', alignItems: 'center',
          fontSize: 13, fontWeight: 600, color: 'var(--skr-text-primary)',
        }}
      >
      ...
      {!collapsed && (
        <div style={{ padding: '14px', background: 'var(--skr-base)', display: 'flex', flexDirection: 'column', gap: 10 }}>
          {/* Character preset */}
          <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
            {fields.map(({ key, label, options }) => (
              <div key={key}>
                <label className="skr-field-label" style={{ fontSize: 11 }}>{label}</label>
                <select
                  className="skr-field-select"
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 4 }}>
            <button className="skr-btn-secondary" style={{ flex: 1, fontSize: 12 }} onClick={onCreate} disabled={isLoading}>
              Build prompt
            </button>
            <button className="skr-btn-primary" style={{ flex: 1, fontSize: 12 }} onClick={onAiGenerate} disabled={isLoading}>
