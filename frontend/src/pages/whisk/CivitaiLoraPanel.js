 ...
 */
export default function CivitaiLoraPanel({
      ...
      {civitaiLoraMode === "profile" ? (
        <div>
          ...
          <select
            ...
            value={selectedLoraProfileId}
            onChange={(e) => onLoraProfileChange(e.target.value)}
          >
            <option value="">— None —</option>
            {loraProfiles.map((p) => (
              <option key={p.id || p.characterId} value={p.id || p.characterId}>
                {p.name || p.displayName || p.id || p.characterId}
              </option>
            ))}
          </select>
        </div>
          ...
          {(civitaiCatalogResults || []).length > 0 && (
            ...
            >
              {civitaiCatalogResults.map((entry) => {
                const already = (civitaiRuntimeLoras || []).some(
                  (r) => r.catalogId === entry.catalogId
                );
                const atLimit =
                  (civitaiRuntimeLoras || []).length >= (civitaiRuntimeLoraLimit || 9);
                return (
                  <div
                    key={entry.catalogId}
                    style={{
                  ...
                  >
                    <div>
                      <span style={{ fontWeight: 600, color: "var(--skr-text-primary)" }}>
                        {entry.name || entry.catalogId}
                      </span>
                      {entry.baseModel && (
                        <span
                          style={{ marginLeft: 6, color: "var(--skr-text-tertiary)", fontSize: 11 }}
                        >
                          {entry.baseModel}
                        </span>
                      )}
          ...
          {(civitaiRuntimeLoras || []).length > 0 && (
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              <p style={{ fontSize: 11, color: "var(--skr-text-tertiary)", margin: 0 }}>
                Active LoRAs ({civitaiRuntimeLoras.length}/{civitaiRuntimeLoraLimit || 9})
              </p>
              {civitaiRuntimeLoras.map((lora) => (
                <div
                  key={lora.catalogId}
                  style={{ background: "var(--skr-surface)", borderRadius: 6, padding: "6px 10px" }}
                >
                  <div
                    style={{
                      display: "flex",
                  ...
                  >
                    <span
                      style={{ fontSize: 12, fontWeight: 600, color: "var(--skr-text-primary)" }}
                    >
                      {lora.name || lora.catalogId}
                    </span>
                    <button
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    ...
                    />
                    <span
                      style={{ fontSize: 11, color: "var(--skr-text-secondary)", minWidth: 30 }}
                    >
