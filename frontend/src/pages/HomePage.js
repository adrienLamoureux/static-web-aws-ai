  ...
  return (
    <div>
      ...
      <div className="kit-page-header">
        <h1 className="kit-page-title">Home</h1>
        <p className="kit-page-subtitle">Your creative dashboard</p>
      </div>
      ...
      {masonryImages.length > 0 && (
        <div className="kit-masonry">
          {masonryImages.map(img => (
            <div key={img.id} className="kit-masonry__item">
              <img
                src={img.src}
                alt=""
                loading="lazy"
      ...
      >
        <div className="kit-stat-card">
          <span className="kit-stat-value">{masonryImages.length}</span>
          <span className="kit-stat-label">Assets</span>
        </div>
        <div className="kit-stat-card">
          <span className="kit-stat-value">{recentImages.length}</span>
          <span className="kit-stat-label">Recent</span>
        </div>
        <div className="kit-stat-card">
          <span className="kit-stat-value">{NAV_CARDS.length}</span>
          <span className="kit-stat-label">Tools</span>
        </div>
      </div>

      {/* Continue creating — navigation cards */}
      <div style={{ marginBottom: 28 }}>
        <h2 className="kit-section-title">Continue creating</h2>
        <div
          style={{
            display: 'grid',
            ...
            >
              ...
              >
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <span style={{ fontSize: 14, fontWeight: 600, color: 'var(--kit-text-primary)' }}>
                    {card.label}
                  </span>
                  <span style={{ fontSize: 12, color: 'var(--kit-accent, var(--kit-text-tertiary))' }}>→</span>
                </div>
                <p style={{ fontSize: 12, color: 'var(--kit-text-secondary)', margin: 0, lineHeight: 1.4 }}>
                  {card.description}
                </p>
              </div>
            </Link>
      ...
      {recentImages.length > 0 && (
        <div style={{ marginTop: 28 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
            <span style={{ fontSize: 12, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--kit-text-tertiary)' }}>
              Recently generated
            </span>
            <Link
              to="/showcase"
              style={{ fontSize: 12, color: 'var(--kit-text-secondary)', textDecoration: 'none' }}
            >
              View all →
            </Link>
          </div>
