    ...
    <div>
      ...
      <div className="yk-masonry">
        {masonryImages.map(img => (
          <img key={img.id} src={img.src} alt="" loading="lazy" />
        ))}
      </div>
            ...
            >
              ...
              >
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <span style={{ fontSize: 14, fontWeight: 600, color: 'var(--yk-text-primary)' }}>
                    {card.label}
                  </span>
                  <span style={{ fontSize: 12, color: 'var(--yk-accent, var(--yk-text-tertiary))' }}>→</span>
                </div>
                <p style={{ fontSize: 12, color: 'var(--yk-text-secondary)', margin: 0, lineHeight: 1.4 }}>
                  {card.description}
                </p>
              </div>
            </Link>
      ...
      {recentImages.length > 0 && (
        <div style={{ marginTop: 28 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
            <span style={{ fontSize: 12, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--yk-text-tertiary)' }}>
              Recently generated
            </span>
            <Link
              to="/showcase"
              style={{ fontSize: 12, color: 'var(--yk-text-secondary)', textDecoration: 'none' }}
            >
              View all →
            </Link>
          </div>
