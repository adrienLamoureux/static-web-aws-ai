...
export default function CompanionSection({ apiBaseUrl }) {
  ...
  const testDone = () => dispatch(CompanionActions.GENERATION_DONE, { type: 'image', success: true });
  const testError = () => dispatch(CompanionActions.GENERATION_ERROR, { type: 'image', error: 'test error' });
  ...
  return (
    <div className="skr-card" style={{ padding: 20, marginBottom: 16 }}>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
        ...
        <button className="skr-btn-primary" style={{ fontSize: 12 }} onClick={handleSave} disabled={isSaving || !selectedModelId}>
          {isSaving ? 'Saving…' : 'Save'}
        </button>
      </div>
      <p style={{ fontSize: 12, fontWeight: 600, color: 'var(--skr-text-secondary)', marginBottom: 6 }}>Test Reactions</p>
      <div style={{ display: 'flex', gap: 6 }}>
        <button className="skr-btn-secondary" style={{ fontSize: 11 }} onClick={testDone}>Test: Done</button>
        <button className="skr-btn-secondary" style={{ fontSize: 11 }} onClick={testError}>Test: Error</button>
        <button className="skr-btn-secondary" style={{ fontSize: 11 }} onClick={testStory}>Test: Story</button>
      </div>
