  ...
  return (
    <div style={{ marginTop: 16, borderTop: "1px solid var(--skr-border)", paddingTop: 12 }}>
      ...
      <div style={{ display: "flex", gap: 6, marginBottom: 8, flexWrap: "wrap" }}>
        <input
          className="skr-input"
          style={{ fontSize: 12, flex: 2, minWidth: 120 }}
          placeholder="User ID"
          value={userId}
      ...
      {memory && (
        <div className="skr-info-list" style={{ fontSize: 11 }}>
          <div>
            Turn count: <strong>{memory.turnCount ?? "—"}</strong>
          </div>
          {memory.updatedAt && (
            <div>
              Last updated: <strong>{formatTimestamp(memory.updatedAt)}</strong>
            </div>
          )}
          {memory.summary && (
            <div style={{ marginTop: 4 }}>
              <span style={{ color: "var(--skr-text-tertiary)" }}>Summary:</span> {memory.summary}
            </div>
          )}
        </div>
      )}
