...
import {
  listSharedImagesAdmin,
  deleteSharedImageAdmin,
  listSharedVideosAdmin,
  deleteSharedVideoAdmin,
  ...
  const load = useCallback(
    async (token = null) => {
      ...
      try {
        ...
        const data = await fetcher(apiBaseUrl, {
          limit: 120,
    ...
    <div style={{ marginBottom: 20 }}>
      ...
      <div className="skr-tab-bar" style={{ marginBottom: 12 }}>
        {TABS.map((t, i) => (
          <button
            key={t}
            className={`skr-tab${tab === i ? " is-active" : ""}`}
            onClick={() => setTab(i)}
          >
            {t}
          </button>
