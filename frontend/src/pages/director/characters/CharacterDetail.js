      ...
      {hasDefaults && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 10 }}>
          {char.defaultImageModel && (
            <div>
              <span style={{ fontSize: 10, fontWeight: 600, color: 'var(--skr-text-tertiary)', textTransform: 'uppercase' }}>Image model</span>
              <p style={{ fontSize: 12, margin: '2px 0 0', color: 'var(--skr-text-secondary)' }}>{char.defaultImageModel}</p>
            </div>
          )}
          {char.defaultVideoModel && (
            <div>
              <span style={{ fontSize: 10, fontWeight: 600, color: 'var(--skr-text-tertiary)', textTransform: 'uppercase' }}>Video model</span>
              <p style={{ fontSize: 12, margin: '2px 0 0', color: 'var(--skr-text-secondary)' }}>{char.defaultVideoModel}</p>
            </div>
          )}
          {char.defaultImagePrompt && (
            <div style={{ gridColumn: '1 / -1' }}>
              <span style={{ fontSize: 10, fontWeight: 600, color: 'var(--skr-text-tertiary)', textTransform: 'uppercase' }}>Image prompt</span>
              <p style={{ fontSize: 11, margin: '2px 0 0', color: 'var(--skr-text-tertiary)', fontFamily: 'monospace', wordBreak: 'break-word' }}>{char.defaultImagePrompt}</p>
            </div>
          )}
          {char.defaultVideoPrompt && (
            <div style={{ gridColumn: '1 / -1' }}>
              <span style={{ fontSize: 10, fontWeight: 600, color: 'var(--skr-text-tertiary)', textTransform: 'uppercase' }}>Video prompt</span>
              <p style={{ fontSize: 11, margin: '2px 0 0', color: 'var(--skr-text-tertiary)', fontFamily: 'monospace', wordBreak: 'break-word' }}>{char.defaultVideoPrompt}</p>
            </div>
          )}
        </div>
      )}
