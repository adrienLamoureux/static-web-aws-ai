...
export default function CharactersSection({ apiBaseUrl, imageModels = [], videoModels = [] }) {
  ...
  const handleToggleExpand = (charId) => setExpandedCharId(prev => (prev === charId ? null : charId));
  const handleStartAddChar = () => { setCharForm(emptyCharForm()); setExpandedCharId(null); };
  const handleStartEditChar = (char) => { setCharForm(charToForm(char)); setExpandedCharId(char.id); };
  const handleCancelCharForm = () => setCharForm(null);
  const handleCharFormChange = (field, value) => setCharForm(prev => ({ ...prev, [field]: value }));
  ...
  return (
    <div style={{ marginTop: 32 }}>
      ...
      {message && <p style={{ fontSize: 12, color: 'var(--skr-accent)', background: 'var(--skr-accent-subtle, rgba(217,119,6,0.1))', padding: '6px 10px', borderRadius: 6, marginBottom: 10 }}>{message}</p>}
      {error && <p style={{ fontSize: 12, color: '#ef4444', background: '#fef2f2', padding: '6px 10px', borderRadius: 6, marginBottom: 10 }}>{error}</p>}
      {charForm && charForm._id === null && (
        <CharacterForm charForm={charForm} onFieldChange={handleCharFormChange} onSave={handleSaveChar} onCancel={handleCancelCharForm} saving={savingChar} imageModels={imageModels} videoModels={videoModels} />
      )}
      {isLoading ? (
                    ...
                    <div key={char.id} className="skr-card" style={{ padding: 16 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer' }} onClick={() => handleToggleExpand(char.id)}>
                        <div style={{ flex: 1 }}>
                          ...
                          {char.signatureTraits && <span style={{ marginLeft: 8, fontSize: 11, color: 'var(--skr-text-tertiary)' }}>{char.signatureTraits}</span>}
                        </div>
