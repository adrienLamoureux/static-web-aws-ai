    ...
    <div style={{ marginTop: 24 }}>
      ...
      <div className="skr-tab-bar" style={{ marginBottom: 8 }}>
        {SESSION_TABS.map((t, i) => (
          <button key={t} className={`skr-tab${activeTab === i ? ' is-active' : ''}`} onClick={() => setActiveTab(i)}>{t}</button>
        ))}
      </div>
      <input
        className="skr-input"
      ...
      <div className="skr-card" style={{ padding: 0, overflow: 'hidden' }}>
        {activeIsLoading ? (
          ...
          filtered.map((session, i) => (
            <div key={session.sessionId || i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 16px', borderBottom: i < filtered.length - 1 ? '1px solid var(--skr-border)' : 'none' }}>
              <div style={{ flex: 1 }}>
                <p style={{ fontSize: 13, fontWeight: 600, color: 'var(--skr-text-primary)', margin: 0 }}>
                  ...
                  {activeTab === 1 && session.userId && (
                    <span className="skr-chip" style={{ marginLeft: 8, fontSize: 10 }}>{session.userId.slice(0, 12)}</span>
                  )}
                </p>
                {session.updatedAt && (
