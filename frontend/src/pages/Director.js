...
export default function Director() {
  ...
  return (
    <div>
      <div className="sol-page-header" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          <h2 className="sol-page-title">Global Command Center</h2>
          <p className="sol-page-subtitle">Director overview and orchestration controls</p>
        </div>
        <button className="sol-btn-secondary" onClick={loadOverview} disabled={isLoading} style={{ fontSize: 12 }}>
      ...
      {/* Summary stat cards */}
      <div className="sol-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: 12, marginBottom: 20 }}>
        {[
          ...
          <div key={label} className="sol-card" style={{ padding: '16px 20px' }}>
            <p style={{ fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.06em', color: 'var(--sol-text-tertiary)', marginBottom: 6 }}>{label}</p>
            <p style={{ fontSize: 28, fontWeight: 700, color: 'var(--sol-text-primary)' }}>{isLoading ? '—' : value}</p>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
        {/* Generation Module */}
        <div className="sol-card" style={{ padding: 20 }}>
          <p style={{ fontSize: 13, fontWeight: 700, color: 'var(--sol-text-primary)', marginBottom: 12 }}>Generation</p>
          {isLoading ? (
            <p style={{ fontSize: 12, color: 'var(--sol-text-tertiary)' }}>Loading…</p>
          ) : (
            <div style={{ fontSize: 12, color: 'var(--sol-text-secondary)', display: 'flex', flexDirection: 'column', gap: 6 }}>
        ...
        {/* Video Module */}
        <div className="sol-card" style={{ padding: 20 }}>
          <p style={{ fontSize: 13, fontWeight: 700, color: 'var(--sol-text-primary)', marginBottom: 12 }}>Video</p>
          {isLoading ? (
            <p style={{ fontSize: 12, color: 'var(--sol-text-tertiary)' }}>Loading…</p>
          ) : (
            <div style={{ fontSize: 12, color: 'var(--sol-text-secondary)', display: 'flex', flexDirection: 'column', gap: 6 }}>
        ...
        {/* Story Module */}
        <div className="sol-card" style={{ padding: 20 }}>
          <p style={{ fontSize: 13, fontWeight: 700, color: 'var(--sol-text-primary)', marginBottom: 12 }}>Story Sessions</p>
          {isLoading ? (
            <p style={{ fontSize: 12, color: 'var(--sol-text-tertiary)' }}>Loading…</p>
          ) : (
            <div style={{ fontSize: 12, color: 'var(--sol-text-secondary)', display: 'flex', flexDirection: 'column', gap: 6 }}>
        ...
        {/* Config */}
        <div className="sol-card" style={{ padding: 20 }}>
          <p style={{ fontSize: 13, fontWeight: 700, color: 'var(--sol-text-primary)', marginBottom: 12 }}>Config</p>
          {isLoading ? (
            <p style={{ fontSize: 12, color: 'var(--sol-text-tertiary)' }}>Loading…</p>
          ) : (
            <div style={{ fontSize: 12, color: 'var(--sol-text-secondary)', display: 'flex', flexDirection: 'column', gap: 6 }}>
              ...
              <button className="sol-btn-primary" style={{ marginTop: 8, fontSize: 12 }} onClick={handleSaveGenConfig} disabled={isSaving || isLoading}>
      ...
      {actionMessage && <p style={{ marginTop: 16, fontSize: 13, color: 'var(--sol-accent)', background: 'var(--sol-accent-subtle)', padding: '8px 12px', borderRadius: 6 }}>{actionMessage}</p>}
  ...
  return (
    <div style={{ marginTop: 24 }}>
      <div className="sol-page-header" style={{ marginBottom: 12 }}>
        <h3 style={{ fontSize: 15, fontWeight: 700, color: 'var(--sol-text-primary)' }}>Story Sessions</h3>
      </div>
      {error && <p style={{ fontSize: 12, color: '#ef4444', marginBottom: 8 }}>{error}</p>}
      <div className="sol-card" style={{ padding: 0, overflow: 'hidden' }}>
        {sessions.map((session, i) => (
          <div key={session.sessionId || i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 16px', borderBottom: i < sessions.length - 1 ? '1px solid var(--sol-border)' : 'none' }}>
            <div style={{ flex: 1 }}>
              <p style={{ fontSize: 13, fontWeight: 600, color: 'var(--sol-text-primary)', margin: 0 }}>
                {session.title || session.name || session.sessionId?.slice(0, 12) || 'Session'}
                {session.pinned && <span className="sol-chip accent" style={{ marginLeft: 8 }}>Pinned</span>}
              </p>
              {session.updatedAt && (
                <p style={{ fontSize: 11, color: 'var(--sol-text-tertiary)', margin: 0 }}>{formatTimestamp(session.updatedAt)}</p>
              )}
            </div>
            <button
...
function JobQueue({ apiBaseUrl, jobs, isLoading, onRefresh }) {
  ...
  return (
    <div style={{ marginTop: 24 }}>
      <div className="sol-page-header" style={{ marginBottom: 12 }}>
        <h3 style={{ fontSize: 15, fontWeight: 700, color: 'var(--sol-text-primary)' }}>Job Queue</h3>
      </div>
      {error && <p style={{ fontSize: 12, color: '#ef4444', marginBottom: 8 }}>{error}</p>}
      <div className="sol-card" style={{ padding: 0, overflow: 'hidden' }}>
        {jobs.map((job, i) => (
          <div key={job.jobId || i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 16px', borderBottom: i < jobs.length - 1 ? '1px solid var(--sol-border)' : 'none' }}>
            <div style={{ flex: 1 }}>
              <p style={{ fontSize: 13, fontWeight: 600, color: 'var(--sol-text-primary)', margin: 0 }}>
                {job.type || job.jobId?.slice(0, 12) || 'Job'}
                <span className={`sol-chip${job.priority === 'high' ? ' accent' : ''}`} style={{ marginLeft: 8 }}>{job.priority || 'normal'}</span>
                <span className="sol-chip" style={{ marginLeft: 4 }}>{job.status || 'queued'}</span>
              </p>
              {job.createdAt && (
                <p style={{ fontSize: 11, color: 'var(--sol-text-tertiary)', margin: 0 }}>{formatTimestamp(job.createdAt)}</p>
              )}
            </div>
  ...
  return (
    <div style={{ marginTop: 28 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
        <div>
          <h3 style={{ fontSize: 15, fontWeight: 700, color: 'var(--sol-text-primary)', margin: 0 }}>Masonry Portraits</h3>
          <p style={{ fontSize: 12, color: 'var(--sol-text-tertiary)', margin: '3px 0 0' }}>
            {isLoading ? 'Loading…' : `${images.length} portrait${images.length !== 1 ? 's' : ''} — shown on the home page hero`}
          </p>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          <button className="sol-btn-secondary" style={{ fontSize: 12 }} onClick={refresh} disabled={isLoading}>
            ↺ Refresh
          </button>
          <button
      ...
      {message && <p style={{ fontSize: 12, color: 'var(--sol-accent)', background: 'var(--sol-accent-subtle)', padding: '6px 10px', borderRadius: 6, marginBottom: 12 }}>{message}</p>}
      {error && <p style={{ fontSize: 12, color: '#ef4444', background: '#fef2f2', padding: '6px 10px', borderRadius: 6, marginBottom: 12 }}>{error}</p>}

      {isLoading ? (
        <p style={{ fontSize: 12, color: 'var(--sol-text-tertiary)' }}>Loading portraits…</p>
      ) : images.length === 0 ? (
        <div className="sol-card" style={{ padding: '24px 20px', textAlign: 'center', color: 'var(--sol-text-tertiary)', fontSize: 13 }}>
          No portraits uploaded yet. Upload portrait images (3:5 aspect ratio works best) to populate the animated home page hero.
        </div>
      ) : (
        <div className="sol-masonry-mgmt-grid">
          {images.map((img, i) => (
            <div key={img.key || i} className="sol-masonry-mgmt-card">
              <img src={img.url} alt="" loading="lazy" />
              <div className="sol-masonry-mgmt-meta">
...
function CharactersSection({ apiBaseUrl, imageModels = [], videoModels = [] }) {
  ...
  const fieldLabel = (label, optional = false) => (
    <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--sol-text-tertiary)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 3 }}>
  ...
  const modelSelect = (value, onChange, models, placeholder = '— Default —') => (
    <select className="sol-input sol-field-select" style={{ fontSize: 12, width: '100%' }} value={value} onChange={e => onChange(e.target.value)}>
  ...
  const renderCharForm = () => (
    <div className="sol-card" style={{ padding: 20, marginBottom: 12, border: '2px solid var(--sol-accent, #d97706)' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
        <p style={{ fontSize: 13, fontWeight: 700, color: 'var(--sol-text-primary)', margin: 0 }}>
          {charForm?._id ? 'Edit Character' : 'New Character'}
        </p>
        <button className="sol-btn-secondary" style={{ fontSize: 11 }} onClick={handleCancelCharForm}>Cancel</button>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        ...
        <div style={{ gridColumn: '1 / -1' }}>
          {fieldLabel('Name')}
          <input className="sol-input" style={{ fontSize: 12, width: '100%' }} value={charForm?.name || ''} onChange={e => handleCharFormChange('name', e.target.value)} placeholder="e.g. Aria Starfall" />
        </div>
        {/* Default Image Model */}
        <div>
        ...
        <div style={{ gridColumn: '1 / -1' }}>
          ...
          <textarea className="sol-input" style={{ fontSize: 12, width: '100%', resize: 'vertical', minHeight: 56 }} value={charForm?.defaultImagePrompt || ''} onChange={e => handleCharFormChange('defaultImagePrompt', e.target.value)} placeholder="Base positive prompt for image generation…" />
        </div>
        {/* Default Video Prompt */}
        <div style={{ gridColumn: '1 / -1' }}>
          {fieldLabel('Default Video Prompt', true)}
          <textarea className="sol-input" style={{ fontSize: 12, width: '100%', resize: 'vertical', minHeight: 56 }} value={charForm?.defaultVideoPrompt || ''} onChange={e => handleCharFormChange('defaultVideoPrompt', e.target.value)} placeholder="Base positive prompt for video generation…" />
        </div>
        {/* Visual attributes */}
        <div>
          {fieldLabel('Signature Traits', true)}
          <input className="sol-input" style={{ fontSize: 12, width: '100%' }} value={charForm?.signatureTraits || ''} onChange={e => handleCharFormChange('signatureTraits', e.target.value)} placeholder="e.g. slender, confident posture" />
        </div>
        <div>
          {fieldLabel('Eye Details', true)}
          <input className="sol-input" style={{ fontSize: 12, width: '100%' }} value={charForm?.eyeDetails || ''} onChange={e => handleCharFormChange('eyeDetails', e.target.value)} placeholder="e.g. violet heterochromia eyes" />
        </div>
        <div>
          {fieldLabel('Hair Details', true)}
          <input className="sol-input" style={{ fontSize: 12, width: '100%' }} value={charForm?.hairDetails || ''} onChange={e => handleCharFormChange('hairDetails', e.target.value)} placeholder="e.g. long silver wavy hair" />
        </div>
        <div>
          {fieldLabel('Outfit / Materials', true)}
          <input className="sol-input" style={{ fontSize: 12, width: '100%' }} value={charForm?.outfitMaterials || ''} onChange={e => handleCharFormChange('outfitMaterials', e.target.value)} placeholder="e.g. flowing white mage robes" />
        </div>
        <div>
          {fieldLabel('Accessories', true)}
          <input className="sol-input" style={{ fontSize: 12, width: '100%' }} value={charForm?.accessories || ''} onChange={e => handleCharFormChange('accessories', e.target.value)} placeholder="e.g. ornate staff, small pouch" />
        </div>
        <div>
          {fieldLabel('Style Reference', true)}
          <input className="sol-input" style={{ fontSize: 12, width: '100%' }} value={charForm?.styleReference || ''} onChange={e => handleCharFormChange('styleReference', e.target.value)} placeholder="e.g. anime style, cel shaded" />
        </div>
        ...
        {promptPreview && (
          <div style={{ gridColumn: '1 / -1' }}>
            ...
            <textarea readOnly className="sol-input" style={{ fontSize: 11, width: '100%', resize: 'none', minHeight: 48, color: 'var(--sol-text-tertiary)', fontFamily: 'monospace' }} value={promptPreview} />
          </div>
        )}
      </div>

      <div style={{ display: 'flex', gap: 8, marginTop: 14 }}>
        <button className="sol-btn-primary" style={{ fontSize: 12 }} onClick={handleSaveChar} disabled={savingChar}>
          {savingChar ? 'Saving…' : (charForm?._id ? 'Update Character' : 'Create Character')}
        </button>
        <button className="sol-btn-secondary" style={{ fontSize: 12 }} onClick={handleCancelCharForm}>Cancel</button>
      </div>
    </div>
  );
  ...
  const renderCharDetail = (char) => {
    ...
    return (
      <div style={{ borderTop: '1px solid var(--sol-border)', marginTop: 12, paddingTop: 12 }}>
        {hasDefaults && (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 10 }}>
            {char.defaultImageModel && (
              <div><span style={{ fontSize: 10, fontWeight: 600, color: 'var(--sol-text-tertiary)', textTransform: 'uppercase' }}>Image model</span><p style={{ fontSize: 12, margin: '2px 0 0', color: 'var(--sol-text-secondary)' }}>{char.defaultImageModel}</p></div>
            )}
            {char.defaultVideoModel && (
              <div><span style={{ fontSize: 10, fontWeight: 600, color: 'var(--sol-text-tertiary)', textTransform: 'uppercase' }}>Video model</span><p style={{ fontSize: 12, margin: '2px 0 0', color: 'var(--sol-text-secondary)' }}>{char.defaultVideoModel}</p></div>
            )}
            {char.defaultImagePrompt && (
              <div style={{ gridColumn: '1 / -1' }}><span style={{ fontSize: 10, fontWeight: 600, color: 'var(--sol-text-tertiary)', textTransform: 'uppercase' }}>Image prompt</span><p style={{ fontSize: 11, margin: '2px 0 0', color: 'var(--sol-text-tertiary)', fontFamily: 'monospace', wordBreak: 'break-word' }}>{char.defaultImagePrompt}</p></div>
            )}
            {char.defaultVideoPrompt && (
              <div style={{ gridColumn: '1 / -1' }}><span style={{ fontSize: 10, fontWeight: 600, color: 'var(--sol-text-tertiary)', textTransform: 'uppercase' }}>Video prompt</span><p style={{ fontSize: 11, margin: '2px 0 0', color: 'var(--sol-text-tertiary)', fontFamily: 'monospace', wordBreak: 'break-word' }}>{char.defaultVideoPrompt}</p></div>
            )}
          </div>
        )}
        {attrs.length > 0 && (
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 10 }}>
            {attrs.map(a => (
              <span key={a.label} style={{ fontSize: 11, background: 'var(--sol-elevated)', border: '1px solid var(--sol-border)', borderRadius: 4, padding: '2px 8px', color: 'var(--sol-text-secondary)' }}>
                <span style={{ color: 'var(--sol-text-tertiary)', fontSize: 10, textTransform: 'uppercase', fontWeight: 600 }}>{a.label}: </span>{a.value}
              </span>
            ))}
          </div>
        )}
        <p style={{ fontSize: 11, color: 'var(--sol-text-tertiary)', fontStyle: 'italic', margin: 0 }}>
  ...
  return (
    <div style={{ marginTop: 32 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
        <div style={{ flex: 1 }}>
          <h3 style={{ fontSize: 15, fontWeight: 700, color: 'var(--sol-text-primary)', margin: 0 }}>Characters</h3>
          <p style={{ fontSize: 12, color: 'var(--sol-text-tertiary)', margin: '3px 0 0' }}>
        ...
        {!charForm && (
          <button className="sol-btn-secondary" style={{ fontSize: 12, whiteSpace: 'nowrap' }} onClick={handleStartAddChar}>
            + New Character
          </button>
        )}
      </div>
      ...
      {/* Feedback */}
      {message && (
        <p style={{ fontSize: 12, color: 'var(--sol-accent)', background: 'var(--sol-accent-subtle, rgba(217,119,6,0.1))', padding: '6px 10px', borderRadius: 6, marginBottom: 10 }}>
          {message}
        </p>
      )}
      {error && (
      ...
      {isLoading ? (
        <p style={{ fontSize: 12, color: 'var(--sol-text-tertiary)' }}>Loading characters…</p>
      ) : (
        <>
          {/* System characters */}
          {systemChars.length > 0 && (
            <div style={{ marginBottom: 16 }}>
              <p style={{ fontSize: 11, fontWeight: 600, color: 'var(--sol-text-tertiary)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 8 }}>System Characters</p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {systemChars.map(char => {
                  const isExpanded = expandedCharId === char.id;
                  return (
                    <div key={char.id} className="sol-card" style={{ padding: 16 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer' }} onClick={() => handleToggleExpand(char.id)}>
                        <div style={{ flex: 1 }}>
                          <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--sol-text-primary)' }}>{char.name}</span>
                          <span style={{ marginLeft: 8, fontSize: 10, color: 'var(--sol-text-tertiary)', background: 'var(--sol-elevated)', border: '1px solid var(--sol-border)', borderRadius: 4, padding: '1px 5px' }}>system</span>
                          {char.signatureTraits && (
                            <span style={{ marginLeft: 8, fontSize: 11, color: 'var(--sol-text-tertiary)' }}>{char.signatureTraits}</span>
                          )}
                        </div>
                        <span style={{ fontSize: 12, color: 'var(--sol-text-tertiary)' }}>{isExpanded ? '▲' : '▼'}</span>
                      </div>
          ...
          {userChars.length > 0 && (
            <div>
              <p style={{ fontSize: 11, fontWeight: 600, color: 'var(--sol-text-tertiary)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 8 }}>My Characters</p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {userChars.map(char => {
                  const isExpanded = expandedCharId === char.id;
                    ...
                    <div key={char.id} className="sol-card" style={{ padding: 16 }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        ...
                        <div style={{ display: 'flex', gap: 4 }}>
                          <button className="sol-btn-secondary" style={{ fontSize: 11 }} onClick={() => handleStartEditChar(char)} disabled={isDeleting}>Edit</button>
                          <button className="sol-btn-secondary" style={{ fontSize: 11, color: '#ef4444' }} onClick={() => handleDeleteChar(char)} disabled={isDeleting || deletingCharId !== ''}>
                            {isDeleting ? '…' : 'Delete'}
                          </button>
                          <span style={{ fontSize: 12, color: 'var(--sol-text-tertiary)', cursor: 'pointer', padding: '0 4px' }} onClick={() => handleToggleExpand(char.id)}>{isExpanded ? '▲' : '▼'}</span>
                        </div>
                      </div>
