  ...
  return (
    <div>
      <div className="sol-page-header">
        <h2 className="sol-page-title">Story Music Library</h2>
      ...
      <div className="sol-lora-grid" style={{ gridTemplateColumns: 'minmax(260px,1fr) 2fr' }}>
        ...
        <div className="sol-lora-panel">
          ...
          <form onSubmit={handleUpload} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div>
              <label className="sol-field-label" htmlFor="music-file">Audio file</label>
              <input id="music-file" type="file" accept="audio/*" className="sol-input" onChange={onFileChange} disabled={isUploading} />
            </div>
            <div>
              <label className="sol-field-label" htmlFor="music-title">Title</label>
              <input id="music-title" className="sol-input" value={title} onChange={e => setTitle(e.target.value)} placeholder="Forest ambience" maxLength={120} disabled={isUploading} />
            </div>
            <div>
              <label className="sol-field-label" htmlFor="music-desc">Description</label>
              <textarea id="music-desc" className="sol-field-textarea" value={description} onChange={e => setDescription(e.target.value)} placeholder="Warm strings, magical atmosphere…" rows={3} maxLength={800} disabled={isUploading} />
            </div>
            <div>
              <label className="sol-field-label" htmlFor="music-tags">Tags (comma-separated)</label>
              <input id="music-tags" className="sol-input" value={tagsInput} onChange={e => setTagsInput(e.target.value)} placeholder="fantasy, calm, exploration" maxLength={240} disabled={isUploading} />
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
              <div>
                <label className="sol-field-label" htmlFor="music-mood">Mood</label>
                <input id="music-mood" className="sol-input" value={mood} onChange={e => setMood(e.target.value)} placeholder="calm" maxLength={40} disabled={isUploading} />
              </div>
              <div>
                <label className="sol-field-label" htmlFor="music-energy">Energy</label>
                <select id="music-energy" className="sol-field-select" value={energy} onChange={e => setEnergy(e.target.value)} disabled={isUploading}>
              ...
              <div>
                <label className="sol-field-label" htmlFor="music-tempo">BPM</label>
                <input id="music-tempo" type="number" min="1" className="sol-input" value={tempoInput} onChange={e => setTempoInput(e.target.value)} placeholder="96" disabled={isUploading} />
              </div>
            </div>
            {uploadError && <p style={{ fontSize: 12, color: '#ef4444', margin: 0 }}>{uploadError}</p>}
            {uploadNotice && <p style={{ fontSize: 12, color: 'var(--sol-accent)', margin: 0 }}>{uploadNotice}</p>}
            <div style={{ display: 'flex', gap: 8 }}>
              <button type="submit" className="sol-btn-primary" disabled={isUploading}>{isUploading ? 'Uploading…' : 'Upload Track'}</button>
              <button type="button" className="sol-btn-secondary" onClick={resetForm} disabled={isUploading}>Reset</button>
            </div>
          </form>
        </div>
        ...
        <div className="sol-lora-panel">
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
            <p className="sol-lora-section-title" style={{ margin: 0 }}>Saved tracks</p>
            <button className="sol-btn-secondary" style={{ padding: '4px 10px', fontSize: 12 }} onClick={() => refreshTracks(searchParams)} disabled={loading}>
              {loading ? 'Loading…' : 'Refresh'}
            </button>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 12 }}>
            <input className="sol-input" value={query} onChange={e => setQuery(e.target.value)} placeholder="Search title, description…" />
            <input className="sol-input" value={searchTags} onChange={e => setSearchTags(e.target.value)} placeholder="Filter tags" />
            <input className="sol-input" value={searchMood} onChange={e => setSearchMood(e.target.value)} placeholder="Filter mood" maxLength={40} />
            <select className="sol-field-select" value={searchEnergy} onChange={e => setSearchEnergy(e.target.value)}>
            ...
            <select className="sol-field-select" style={{ gridColumn: '1 / -1' }} value={searchSource} onChange={e => setSearchSource(e.target.value)}>
              <option value="">Source: all</option>
              <option value="upload">Source: upload</option>
              <option value="generated">Source: generated</option>
            </select>
          </div>
          <p style={{ fontSize: 12, color: 'var(--sol-text-tertiary)', marginBottom: 12 }}>
            {loading ? 'Searching…' : `${totalMatches} track(s) matched`}
          </p>
          <div>
            {tracks.length === 0 ? (
              <div style={{ textAlign: 'center', padding: 24, color: 'var(--sol-text-tertiary)', fontSize: 13 }}>
