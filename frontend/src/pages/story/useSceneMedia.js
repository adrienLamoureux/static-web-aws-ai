...
export default function useSceneMedia(apiBaseUrl) {
  ...
  // --- animation ---
  const triggerAnimation = useCallback(async (sessionId, sceneId) => {
    ...
    const poll = async (predictionId) => {
      try {
        const statusData = await getStorySceneAnimationStatus(apiBaseUrl, sessionId, sceneId, { predictionId });
        applyData(statusData);
        const st = statusData?.status || '';
        if (st === 'succeeded' || st === 'failed' || st === 'canceled') { finish(); return; }
        animPollTimers.current[sceneId] = setTimeout(() => poll(predictionId), 5000);
      } catch { finish(); }
    };

    try {
      const data = await startStorySceneAnimation(apiBaseUrl, sessionId, sceneId, { prompt: 'A lot of movements' });
      applyData(data);
      const st = data?.status || '';
      if (st === 'succeeded' || st === 'failed' || st === 'canceled') { finish(); return; }
      if (!data?.predictionId) { finish(); return; }
      animPollTimers.current[sceneId] = setTimeout(() => poll(data.predictionId), 5000);
    } catch { finish(); updateScene(sceneId, { videoStatus: 'failed' }); }
  }, [apiBaseUrl, mediaMap, clearAnimPoll, updateScene]);

  // --- music ---
  const triggerMusic = useCallback(async (sessionId, sceneId) => {
    if (!apiBaseUrl || !sessionId || !sceneId) return;
    const current = mediaMap[sceneId];
    ...
    const poll = async (predictionId) => {
      try {
        const statusData = await getStorySceneMusicStatus(apiBaseUrl, sessionId, sceneId, { predictionId });
        applyData(statusData);
        const st = statusData?.status || '';
        if (st === 'succeeded' || st === 'failed' || st === 'canceled') { finish(); return; }
    ...
    } catch { finish(); updateScene(sceneId, { musicStatus: 'failed' }); }
