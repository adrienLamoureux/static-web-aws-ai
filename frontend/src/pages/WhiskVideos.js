      ...
      {videos.length === 0 ? (
        ...
        <div className="sol-masonry">
          {videos.map((video) => {
            ...
            return (
              <div key={video.key} className="sol-card" style={{ padding: 0, overflow: 'hidden', marginBottom: 16 }}>
                <div style={{ position: 'relative', aspectRatio: '16/9', background: 'var(--sol-elevated)' }}>
                  ...
                  <div style={{ position: 'absolute', top: 8, right: 8, display: 'flex', gap: 4 }}>
                    ...
                    <button
                      className={`sol-icon-btn${video.favorite ? ' is-favorite' : ''}`}
                ...
                {video.prompt && (
                  <p style={{ fontSize: 12, color: 'var(--sol-text-secondary)', padding: '8px 12px', margin: 0 }}>
                    {video.prompt}
                  </p>
                )}
              </div>
