...
export default function WhiskVideos() {
  ...
  return (
    <div>
      ...
      {shareNotice && <p style={{ fontSize: 12, color: 'var(--yk-accent)', marginBottom: 12 }}>{shareNotice}</p>}
      ...
      {videos.length === 0 ? (
        ...
        <div className="yk-masonry">
          {videos.map((video) => {
            ...
            return (
              <div key={video.key} className="yk-card" style={{ padding: 0, overflow: 'hidden', marginBottom: 16 }}>
                <div style={{ position: 'relative', aspectRatio: '16/9', background: 'var(--yk-elevated)' }}>
                  ...
                  <div style={{ position: 'absolute', top: 8, right: 8, display: 'flex', gap: 4 }}>
                    ...
                    <button
                      className={`yk-icon-btn${video.favorite ? ' is-favorite' : ''}`}
                ...
                {video.prompt && (
                  <p style={{ fontSize: 12, color: 'var(--yk-text-secondary)', padding: '8px 12px', margin: 0 }}>
                    {video.prompt}
                  </p>
                )}
              </div>
