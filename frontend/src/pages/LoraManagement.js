    ...
    return (
      <div key={modality} className="skr-lora-section">
        ...
        <label style={{ display: 'block', marginBottom: 10 }}>
          ...
          <select className="skr-field-select" value={modalityDraft.modelKey} onChange={(e) => setModalityValue(modality, { modelKey: e.target.value })}>
        ...
        <label style={{ display: 'block', marginBottom: 10 }}>
          ...
          <textarea className="skr-field-textarea" rows={3} value={modalityDraft.promptPrefix} onChange={(e) => setModalityValue(modality, { promptPrefix: e.target.value })} placeholder="cinematic anime key visual, consistent character identity" />
        </label>
              ...
              return (
                <div key={`${modality}-${loraKey}`} style={{ padding: '10px 0', borderBottom: '1px solid var(--skr-border)' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    ...
                    <button type="button" className="skr-btn-secondary" style={{ fontSize: 11, padding: '3px 10px' }} onClick={() => removeProfileLora(modality, entry.catalogId)}>Remove</button>
                  </div>
                  <label style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 6, fontSize: 12 }}>
                    ...
                    <input type="number" step={PROFILE_STRENGTH_STEP} min={PROFILE_STRENGTH_MIN} max={PROFILE_STRENGTH_MAX} className="skr-input" style={{ width: 80 }} value={entry.strength} onChange={(e) => updateProfileLoraStrength(modality, entry.catalogId, e.target.value)} />
                  </label>
                  {entry.triggerWords.length > 0 && (
                    <div style={{ marginTop: 6, display: 'flex', flexWrap: 'wrap', gap: 3 }}>
                      {entry.triggerWords.map((word) => <span key={`${loraKey}-${word}`} className="skr-lora-chip">{word}</span>)}
                    </div>
                  )}
                </div>
              );
      ...
      <div className="skr-lora-grid">
        <div className="skr-lora-panel">
          <div style={{ marginBottom: 16 }}>
            <p style={{ fontSize: 15, fontWeight: 700, color: 'var(--skr-text-primary)', marginBottom: 4 }}>LoRA Catalog</p>
            <p style={{ fontSize: 12, color: 'var(--skr-text-secondary)' }}>Sync LoRA entries from CivitAI, then attach them to character profiles.</p>
          </div>
          ...
          <form onSubmit={handleSync} style={{ marginBottom: 16 }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              <label>
                ...
                <input className="skr-input" style={{ width: '100%' }} value={syncQuery} onChange={(e) => setSyncQuery(e.target.value)} placeholder="frieren" maxLength={120} />
              </label>
              <label>
                <span className="skr-field-label">Base model filter</span>
                <input className="skr-input" style={{ width: '100%' }} value={syncBaseModel} onChange={(e) => setSyncBaseModel(e.target.value)} placeholder="SDXL 1.0" maxLength={120} />
              </label>
              <div style={{ display: 'flex', gap: 12, alignItems: 'flex-end' }}>
                <label style={{ flex: 1 }}>
                  <span className="skr-field-label">Sync limit</span>
                  <input type="number" min={SYNC_MIN_LIMIT} max={SYNC_MAX_LIMIT} className="skr-input" style={{ width: '100%' }} value={syncLimit} onChange={(e) => setSyncLimit(e.target.value)} />
                </label>
              ...
              <div className="skr-lora-actions">
                <button type="submit" className="skr-btn-primary" disabled={isSyncing}>{isSyncing ? "Syncing..." : "Sync CivitAI"}</button>
                <button type="button" className="skr-btn-secondary" onClick={() => loadCatalog({ query: catalogQuery, limit: catalogLimit })} disabled={isCatalogLoading}>Refresh</button>
              </div>
            </div>
          </form>
          ...
          <div style={{ marginBottom: 12 }}>
            <label style={{ display: 'block', marginBottom: 8 }}>
              ...
              <input type="search" className="skr-input" style={{ width: '100%' }} value={catalogQuery} onChange={(e) => setCatalogQuery(e.target.value)} placeholder="Search by name, trigger, creator" />
            </label>
          ...
          <div>
            {catalogItems.length === 0 && !isCatalogLoading ? (
              <p style={{ fontSize: 12, color: 'var(--skr-text-tertiary)' }}>No LoRA catalog entries. Run a sync from CivitAI.</p>
            ) : (
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                      <div>
                        ...
                        <p className="skr-lora-item-meta">{item.baseModel || "Base model n/a"} • {item.creatorName || "Creator n/a"}</p>
                      </div>
                      {item.modelUrl && (
                        <a href={item.modelUrl} target="_blank" rel="noreferrer" className="skr-btn-secondary" style={{ fontSize: 11, padding: '3px 8px', textDecoration: 'none' }}>Open</a>
                      )}
                    </div>
                    <p style={{ fontSize: 10, color: 'var(--skr-text-tertiary)', marginTop: 2 }}>{item.catalogId}</p>
                    {(item.triggerWords || []).length > 0 && (
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 3, marginTop: 4 }}>
                        {(item.triggerWords || []).slice(0, 8).map((word) => <span key={`${item.catalogId}-${word}`} className="skr-lora-chip">{word}</span>)}
                      </div>
                    )}
                    ...
                    <div className="skr-lora-actions" style={{ marginTop: 6 }}>
                      <button type="button" className="skr-btn-secondary" style={{ fontSize: 11, padding: '3px 8px' }} onClick={() => addCatalogItemToProfile(LORA_MODALITY_IMAGE, item)} disabled={!selectedCharacterId}>Add to Image</button>
                      <button type="button" className="skr-btn-secondary" style={{ fontSize: 11, padding: '3px 8px' }} onClick={() => addCatalogItemToProfile(LORA_MODALITY_VIDEO, item)} disabled={!selectedCharacterId}>Add to Video</button>
                    </div>
                  </div>
                </div>
              ))
        ...
        <div className="skr-lora-panel">
          <div style={{ marginBottom: 16 }}>
            ...
            <p style={{ fontSize: 12, color: 'var(--skr-text-secondary)' }}>Attach catalog LoRAs per character and modality. Each character can have multiple named profiles.</p>
          </div>

          {/* Character selector */}
          <label style={{ display: 'block', marginBottom: 10 }}>
            <span className="skr-field-label">Character</span>
            <select className="skr-field-select" value={selectedCharacterId} onChange={(e) => setSelectedCharacterId(e.target.value)} disabled={isBootstrapping}>
          ...
          {selectedCharacterId && (
            <div style={{ marginBottom: 10 }}>
              ...
              {isProfileListLoading && <p style={{ fontSize: 11, color: 'var(--skr-text-tertiary)', marginTop: 4 }}>Loading profiles…</p>}
            </div>
          )}
          ...
          <div className="skr-lora-actions" style={{ marginTop: 16 }}>
            <button type="button" className="skr-btn-primary" onClick={handleSaveProfile} disabled={!selectedCharacterId || isSaving}>
              {isSaving ? "Saving..." : (selectedProfileId ? "Save Profile" : "Create Profile")}
            </button>
            <button type="button" className="skr-btn-secondary" onClick={() => selectedProfileId ? loadProfile(selectedProfileId) : setProfileDraft(createEmptyProfileDraft({ characterId: selectedCharacterId }))} disabled={!selectedCharacterId || isProfileLoading}>
              Reset
            </button>
          </div>
        </div>
