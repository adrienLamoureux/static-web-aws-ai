...
export default function LoraManagement() {
  ...
  const renderModalityEditor = ({ modality, label, modelOptions }) => {
    ...
    return (
      <div key={modality} className="sol-lora-section">
        <p className="sol-lora-section-title">{label}</p>
        <label style={{ display: 'block', marginBottom: 10 }}>
          <span className="sol-field-label">Preferred model key</span>
          <select className="sol-field-select" value={modalityDraft.modelKey} onChange={(e) => setModalityValue(modality, { modelKey: e.target.value })}>
            <option value="">No model lock</option>
            {modelOptions.map((option) => <option key={option.key} value={option.key}>{option.label}</option>)}
          </select>
        </label>
        <label style={{ display: 'block', marginBottom: 10 }}>
          <span className="sol-field-label">Prompt prefix</span>
          <textarea className="sol-field-textarea" rows={3} value={modalityDraft.promptPrefix} onChange={(e) => setModalityValue(modality, { promptPrefix: e.target.value })} placeholder="cinematic anime key visual, consistent character identity" />
        </label>
        <div style={{ marginBottom: 8 }}>
          {(modalityDraft.loras || []).length === 0 ? (
            <p style={{ fontSize: 12, color: 'var(--sol-text-tertiary)' }}>No LoRAs selected for this modality yet.</p>
          ) : (
            modalityDraft.loras.map((entry) => {
              const loraKey = normalizeString(entry.catalogId) || normalizeString(entry.downloadUrl);
              return (
                <div key={`${modality}-${loraKey}`} style={{ padding: '10px 0', borderBottom: '1px solid var(--sol-border)' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    <div>
                      <p className="sol-lora-item-name">{entry.name || entry.catalogId}</p>
                      <p className="sol-lora-item-meta">{entry.catalogId}</p>
                    </div>
                    <button type="button" className="sol-btn-secondary" style={{ fontSize: 11, padding: '3px 10px' }} onClick={() => removeProfileLora(modality, entry.catalogId)}>Remove</button>
                  </div>
                  <label style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 6, fontSize: 12 }}>
                    <span className="sol-field-label" style={{ marginBottom: 0 }}>Strength</span>
                    <input type="number" step={PROFILE_STRENGTH_STEP} min={PROFILE_STRENGTH_MIN} max={PROFILE_STRENGTH_MAX} className="sol-input" style={{ width: 80 }} value={entry.strength} onChange={(e) => updateProfileLoraStrength(modality, entry.catalogId, e.target.value)} />
                  </label>
                  {entry.triggerWords.length > 0 && (
                    <div style={{ marginTop: 6, display: 'flex', flexWrap: 'wrap', gap: 3 }}>
                      {entry.triggerWords.map((word) => <span key={`${loraKey}-${word}`} className="sol-lora-chip">{word}</span>)}
                    </div>
                  )}
                </div>
              );
  ...
  return (
    <div>
      <div className="sol-page-header">
        <h2 className="sol-page-title">LoRA Management</h2>
        <p className="sol-page-subtitle">Character LoRA Profile</p>
      </div>

      <div className="sol-lora-grid">
        <div className="sol-lora-panel">
          <div style={{ marginBottom: 16 }}>
            <p style={{ fontSize: 15, fontWeight: 700, color: 'var(--sol-text-primary)', marginBottom: 4 }}>LoRA Catalog</p>
            <p style={{ fontSize: 12, color: 'var(--sol-text-secondary)' }}>Sync LoRA entries from CivitAI, then attach them to character profiles.</p>
          </div>

          <form onSubmit={handleSync} style={{ marginBottom: 16 }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              <label>
                <span className="sol-field-label">Search query</span>
                <input className="sol-input" style={{ width: '100%' }} value={syncQuery} onChange={(e) => setSyncQuery(e.target.value)} placeholder="frieren" maxLength={120} />
              </label>
              <label>
                <span className="sol-field-label">Base model filter</span>
                <input className="sol-input" style={{ width: '100%' }} value={syncBaseModel} onChange={(e) => setSyncBaseModel(e.target.value)} placeholder="SDXL 1.0" maxLength={120} />
              </label>
              <div style={{ display: 'flex', gap: 12, alignItems: 'flex-end' }}>
                <label style={{ flex: 1 }}>
                  <span className="sol-field-label">Sync limit</span>
                  <input type="number" min={SYNC_MIN_LIMIT} max={SYNC_MAX_LIMIT} className="sol-input" style={{ width: '100%' }} value={syncLimit} onChange={(e) => setSyncLimit(e.target.value)} />
                </label>
              ...
              <div className="sol-lora-actions">
                <button type="submit" className="sol-btn-primary" disabled={isSyncing}>{isSyncing ? "Syncing..." : "Sync CivitAI"}</button>
                <button type="button" className="sol-btn-secondary" onClick={() => loadCatalog({ query: catalogQuery, limit: catalogLimit })} disabled={isCatalogLoading}>Refresh</button>
              </div>
            </div>
          </form>
          ...
          <div style={{ marginBottom: 12 }}>
            <label style={{ display: 'block', marginBottom: 8 }}>
              <span className="sol-field-label">Search local catalog</span>
              <input type="search" className="sol-input" style={{ width: '100%' }} value={catalogQuery} onChange={(e) => setCatalogQuery(e.target.value)} placeholder="Search by name, trigger, creator" />
            </label>
            <p style={{ fontSize: 11, color: 'var(--sol-text-tertiary)' }}>
          ...
          <div>
            {catalogItems.length === 0 && !isCatalogLoading ? (
              <p style={{ fontSize: 12, color: 'var(--sol-text-tertiary)' }}>No LoRA catalog entries. Run a sync from CivitAI.</p>
            ) : (
              catalogItems.map((item) => (
                <div key={item.catalogId} className="sol-lora-catalog-item">
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                      <div>
                        <p className="sol-lora-item-name">{item.name || item.catalogId}</p>
                        <p className="sol-lora-item-meta">{item.baseModel || "Base model n/a"} • {item.creatorName || "Creator n/a"}</p>
                      </div>
                      {item.modelUrl && (
                        <a href={item.modelUrl} target="_blank" rel="noreferrer" className="sol-btn-secondary" style={{ fontSize: 11, padding: '3px 8px', textDecoration: 'none' }}>Open</a>
                      )}
                    </div>
                    <p style={{ fontSize: 10, color: 'var(--sol-text-tertiary)', marginTop: 2 }}>{item.catalogId}</p>
                    {(item.triggerWords || []).length > 0 && (
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 3, marginTop: 4 }}>
                        {(item.triggerWords || []).slice(0, 8).map((word) => <span key={`${item.catalogId}-${word}`} className="sol-lora-chip">{word}</span>)}
                      </div>
                    )}
                    <p style={{ fontSize: 11, color: 'var(--sol-text-tertiary)', marginTop: 4 }}>
                    ...
                    <div className="sol-lora-actions" style={{ marginTop: 6 }}>
                      <button type="button" className="sol-btn-secondary" style={{ fontSize: 11, padding: '3px 8px' }} onClick={() => addCatalogItemToProfile(LORA_MODALITY_IMAGE, item)} disabled={!selectedCharacterId}>Add to Image</button>
                      <button type="button" className="sol-btn-secondary" style={{ fontSize: 11, padding: '3px 8px' }} onClick={() => addCatalogItemToProfile(LORA_MODALITY_VIDEO, item)} disabled={!selectedCharacterId}>Add to Video</button>
                    </div>
                  </div>
                </div>
              ))
        ...
        <div className="sol-lora-panel">
          <div style={{ marginBottom: 16 }}>
            <p style={{ fontSize: 15, fontWeight: 700, color: 'var(--sol-text-primary)', marginBottom: 4 }}>Character LoRA Profile</p>
            <p style={{ fontSize: 12, color: 'var(--sol-text-secondary)' }}>Attach catalog LoRAs per character and modality. Each character can have multiple named profiles.</p>
          </div>

          {/* Character selector */}
          <label style={{ display: 'block', marginBottom: 10 }}>
            <span className="sol-field-label">Character</span>
            <select className="sol-field-select" value={selectedCharacterId} onChange={(e) => setSelectedCharacterId(e.target.value)} disabled={isBootstrapping}>
          ...
          {selectedCharacterId && (
            <div style={{ marginBottom: 10 }}>
              <span className="sol-field-label">Profile</span>
              ...
              {isProfileListLoading && <p style={{ fontSize: 11, color: 'var(--sol-text-tertiary)', marginTop: 4 }}>Loading profiles…</p>}
            </div>
          )}

          {/* Profile name */}
          <label style={{ display: 'block', marginBottom: 16 }}>
            <span className="sol-field-label">Profile name</span>
            <input
              className="sol-input"
              style={{ width: '100%' }}
          ...
          {isProfileLoading ? (
            <p style={{ fontSize: 12, color: 'var(--sol-text-tertiary)' }}>Loading profile...</p>
          ) : (
            <>
          ...
          <div className="sol-lora-actions" style={{ marginTop: 16 }}>
            <button type="button" className="sol-btn-primary" onClick={handleSaveProfile} disabled={!selectedCharacterId || isSaving}>
              {isSaving ? "Saving..." : (selectedProfileId ? "Save Profile" : "Create Profile")}
            </button>
            <button type="button" className="sol-btn-secondary" onClick={() => selectedProfileId ? loadProfile(selectedProfileId) : setProfileDraft(createEmptyProfileDraft({ characterId: selectedCharacterId }))} disabled={!selectedCharacterId || isProfileLoading}>
              Reset
            </button>
          </div>
        </div>
      ...
      {notice && <p style={{ marginTop: 16, fontSize: 13, color: 'var(--sol-accent)', background: 'var(--sol-accent-subtle)', padding: '8px 12px', borderRadius: 6 }}>{notice}</p>}
