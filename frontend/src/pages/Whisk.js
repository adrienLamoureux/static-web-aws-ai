...
function PromptHelperPanel({ promptHelperProps, collapsed, onToggle }) {
  ...
  return (
    <div style={{ border: '1px solid var(--kit-border)', borderRadius: 8, overflow: 'hidden' }}>
      <button
        type="button"
        onClick={onToggle}
        style={{
          width: '100%', textAlign: 'left', padding: '10px 14px',
          background: 'var(--kit-surface)', border: 'none', cursor: 'pointer',
          display: 'flex', justifyContent: 'space-between', alignItems: 'center',
          fontSize: 13, fontWeight: 600, color: 'var(--kit-text-primary)',
        }}
      >
      ...
      {!collapsed && (
        <div style={{ padding: '14px', background: 'var(--kit-base)', display: 'flex', flexDirection: 'column', gap: 10 }}>
          {/* Character preset */}
          <div>
            <label className="kit-field-label">Character preset</label>
            <select
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
            {fields.map(({ key, label, options }) => (
              <div key={key}>
                <label className="kit-field-label" style={{ fontSize: 11 }}>{label}</label>
                <select
                  className="kit-field-select"
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 4 }}>
            <button className="kit-btn-secondary" style={{ flex: 1, fontSize: 12 }} onClick={onCreate} disabled={isLoading}>
              Build prompt
            </button>
            <button className="kit-btn-primary" style={{ flex: 1, fontSize: 12 }} onClick={onAiGenerate} disabled={isLoading}>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      ...
      <div style={{ display: 'flex', gap: 6 }}>
        <button
          className={civitaiLoraMode === 'profile' ? 'kit-btn-primary' : 'kit-btn-secondary'}
        ...
        <button
          className={civitaiLoraMode === 'quick' ? 'kit-btn-primary' : 'kit-btn-secondary'}
          style={{ fontSize: 12, flex: 1 }}
      ...
      {civitaiLoraMode === 'profile' ? (
        <div>
          <label className="kit-field-label">LoRA Profile</label>
          <select className="kit-field-select" value={selectedLoraProfileId} onChange={e => onLoraProfileChange(e.target.value)}>
            <option value="">— None —</option>
            {loraProfiles.map(p => (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {/* Catalog search */}
          <div>
            <label className="kit-field-label">Search CivitAI catalog</label>
            <input
              className="kit-input"
              ...
              {civitaiCatalogResults.map(entry => {
                ...
                return (
                  <div key={entry.catalogId} style={{
                    display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                    padding: '6px 8px', background: 'var(--kit-surface)', borderRadius: 6, fontSize: 12,
                  }}>
                    <div>
                      <span style={{ fontWeight: 600, color: 'var(--kit-text-primary)' }}>{entry.name || entry.catalogId}</span>
                      {entry.baseModel && <span style={{ marginLeft: 6, color: 'var(--kit-text-tertiary)', fontSize: 11 }}>{entry.baseModel}</span>}
                    </div>
                    <button
          ...
          {(civitaiRuntimeLoras || []).length > 0 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              <p style={{ fontSize: 11, color: 'var(--kit-text-tertiary)', margin: 0 }}>Active LoRAs ({civitaiRuntimeLoras.length}/{civitaiRuntimeLoraLimit || 9})</p>
              {civitaiRuntimeLoras.map(lora => (
                <div key={lora.catalogId} style={{ background: 'var(--kit-surface)', borderRadius: 6, padding: '6px 10px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                    <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--kit-text-primary)' }}>{lora.name || lora.catalogId}</span>
                    <button
                      onClick={() => onRemoveCivitaiRuntimeLora(lora.catalogId)}
                      style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--kit-text-tertiary)', fontSize: 14 }}
                    >✕</button>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <input
                      ...
                      className="kit-strength-slider"
                      style={{ flex: 1 }}
                    />
                    <span style={{ fontSize: 11, color: 'var(--kit-text-secondary)', minWidth: 30 }}>{(lora.strength ?? 0.8).toFixed(2)}</span>
                  </div>
                </div>
              ))}
            </div>
...
export default function Whisk() {
  ...
  return (
    <div className="kit-split" style={{ height: 'calc(100vh - 112px)' }}>

      {/* ── Left: image gallery ── */}
      <div className="kit-split-canvas">
        ...
        {promptPreviewImage && (
          <div className="kit-modal-backdrop" onClick={() => setPromptPreviewImage(null)}>
            <div className="kit-modal" onClick={e => e.stopPropagation()}>
              <div className="kit-modal-header">
                <span className="kit-modal-title">Prompt</span>
                <button className="kit-modal-close" onClick={() => setPromptPreviewImage(null)}>✕</button>
              </div>
              <p style={{ fontSize: 13, color: 'var(--kit-text-secondary)', lineHeight: 1.6 }}>
      ...
      <div className="kit-split-controls">
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14, height: '100%', overflowY: 'auto' }}>

          {/* Source tabs */}
          <div>
            <label className="kit-field-label">Source</label>
            <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
              {(imageSourceOptions || []).map(opt => (
                <button
                  key={opt.key}
                  className={imageSource === opt.key ? 'kit-btn-primary' : 'kit-btn-secondary'}
          ...
          {imageSource !== 'upload' ? (
            <>
              {/* Image name */}
              <div>
                <label className="kit-field-label">Name <span style={{ color: 'var(--kit-text-tertiary)', fontWeight: 400 }}>(optional)</span></label>
                <input
                  className="kit-input"
              ...
              {/* Model */}
              <div>
                <label className="kit-field-label">Model</label>
                <select className="kit-field-select" value={imageModel} onChange={e => onSelectModel(e.target.value)}>
              ...
              {/* Size */}
              <div>
                <label className="kit-field-label">Size</label>
                <select className="kit-field-select" value={imageSize} onChange={e => onImageSizeChange(e.target.value)}>
              ...
              {/* Prompt */}
              <div>
                <label className="kit-field-label">Prompt</label>
                <textarea
                  className="kit-input"
                  rows={4}
              ...
              {/* Notice */}
              {imageGenerationNotice && (
                <p style={{ fontSize: 11, color: 'var(--kit-text-secondary)', margin: 0 }}>{imageGenerationNotice}</p>
              )}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div>
                <label className="kit-field-label">Image name</label>
                <input
                  className="kit-input"
              ...
              <div>
                <label className="kit-field-label">File</label>
                <input
                  type="file"
                  accept="image/*"
                  onChange={imageUploadProps.onFileChange}
                  style={{ fontSize: 13, color: 'var(--kit-text-primary)' }}
                />
              </div>
      ...
      {activeModal === 'generate' && (
        <div className="kit-modal-backdrop" onClick={closeModal}>
          <div
            className="kit-modal"
          ...
          >
            <div className="kit-modal-header">
              <span className="kit-modal-title">Image Generation</span>
              <button className="kit-modal-close" onClick={closeModal}>✕</button>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              {imageSource !== 'upload' ? (
                <>
                  {/* Generation name */}
                  <div>
                    <label className="kit-field-label">Generation name <span style={{ color: 'var(--kit-text-tertiary)', fontWeight: 400 }}>(optional)</span></label>
                    <input
                      className="kit-input"
                      placeholder="e.g. frieren-forest-scene"
                      value={imageGenerationName}
                  ...
                  {/* Negative prompt */}
                  <div>
                    <label className="kit-field-label">Negative prompt</label>
                    <textarea
                      className="kit-input"
                      rows={3}
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
                    ...
                    <div>
                      <label className="kit-field-label">Count</label>
                      <select className="kit-field-select" value={imageCount} onChange={e => onImageCountChange(e.target.value)}>
                        {(imageCountOptions || []).map(o => <option key={o.value} value={o.value}>{o.label || o.value}</option>)}
                      </select>
                    </div>
                    <div>
                      <label className="kit-field-label">Scheduler</label>
                      <select className="kit-field-select" value={imageScheduler} onChange={e => onImageSchedulerChange(e.target.value)}>
                  ...
                  {imageSource === 'civitai' && (
                    <div>
                      <label className="kit-field-label" style={{ marginBottom: 8 }}>LoRA configuration</label>
                      <CivitaiLoraPanel
                        imageGenerationProps={imageGenerationProps}
                        loraProfiles={loraProfiles}
                        selectedLoraProfileId={selectedLoraProfileId}
                  ...
                  {/* Notice / error */}
                  {imageGenerationNotice && (
                    <p style={{ fontSize: 12, color: 'var(--kit-text-secondary)', margin: 0 }}>{imageGenerationNotice}</p>
                  )}
                  {error && <p style={{ fontSize: 12, color: '#ef4444', margin: 0 }}>{error}</p>}
                  ...
                  <div style={{ display: 'flex', gap: 8, paddingTop: 4 }}>
                    ...
                    <button className="kit-btn-secondary" onClick={closeModal} disabled={isGeneratingImage}>Cancel</button>
                  </div>
                </>
              ) : (
                /* Upload tab in modal */
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  ...
                  <div>
                    <label className="kit-field-label">File</label>
                    <input type="file" accept="image/*" onChange={imageUploadProps.onFileChange} style={{ fontSize: 13, color: 'var(--kit-text-primary)' }} />
                  </div>
                  {imageUploadProps.previewUrl && (
                    <img src={imageUploadProps.previewUrl} alt="preview" style={{ maxWidth: '100%', borderRadius: 8, maxHeight: 200, objectFit: 'contain' }} />
                  )}
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    <button className="kit-btn-primary" style={{ flex: 1 }} onClick={imageUploadProps.onUpload} disabled={isUploading || !imageUploadProps.selectedFile}>
                      {isUploading ? 'Uploading…' : 'Upload'}
                    </button>
                    <button className="kit-btn-secondary" onClick={closeModal} disabled={isUploading}>Cancel</button>
                  </div>
                </div>
              )}
            </div>
      ...
      {activeModal === 'video' && (
        <div className="kit-modal-backdrop" onClick={() => setActiveModal('')}>
          <div className="kit-modal" style={{ width: 480, maxWidth: '95vw' }} onClick={e => e.stopPropagation()}>
            <div className="kit-modal-header">
              <span className="kit-modal-title">Generate Video</span>
              <button className="kit-modal-close" onClick={() => setActiveModal('')}>✕</button>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              {/* Provider */}
              <div>
                <label className="kit-field-label">Provider</label>
                <div style={{ display: 'flex', gap: 6 }}>
                  {(videoProviderOptions || []).map(opt => (
                    <button
                      key={opt.key}
                      className={videoProvider === opt.key ? 'kit-btn-primary' : 'kit-btn-secondary'}
              ...
              {/* Video model */}
              <div>
                <label className="kit-field-label">Video model</label>
                <select className="kit-field-select" value={videoModel} onChange={e => setVideoModel(e.target.value)}>
              ...
              {/* Motion prompt */}
              <div>
                <label className="kit-field-label">Motion prompt</label>
                <textarea
                  className="kit-input"
                  rows={3}
              ...
              {isReplicateAudioOption && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  />
                  <label htmlFor="vid-audio" className="kit-field-label" style={{ margin: 0, cursor: 'pointer' }}>Generate audio</label>
                </div>
              )}
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                <button className="kit-btn-secondary" onClick={() => setActiveModal('')} disabled={isGeneratingVideo || isVideoInProgress}>
                  Cancel
                </button>
              </div>

              {(isGeneratingVideo || isVideoInProgress) && (
                <p style={{ fontSize: 12, color: 'var(--kit-text-secondary)', textAlign: 'center' }}>
