    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 112px)' }}>
      ...
      <div className="kit-page-header" style={{ marginBottom: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
          <div style={{ flex: 1 }}>
            <h2 className="kit-page-title">Storytelling Studio</h2>
            <p className="kit-page-subtitle">AI-assisted narrative creation</p>
          </div>
          {sessions.length > 0 && (
            <select
              className="kit-input"
          ...
          <button className="kit-btn-secondary" style={{ whiteSpace: 'nowrap' }} onClick={handleNewSession}>
            + New Session
          </button>
        </div>
      </div>
      ...
      {activeSessionId && (sessionCharacterId || sessionLoraProfileId) && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10, flexWrap: 'wrap', position: 'relative' }}>
          {sessionCharacterId && (
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 5, background: 'var(--kit-elevated)', border: '1px solid var(--kit-border)', borderRadius: 6, padding: '3px 10px', fontSize: 12 }}>
              <span style={{ fontSize: 10, fontWeight: 700, color: 'var(--kit-text-tertiary)', textTransform: 'uppercase' }}>Character</span>
              <span style={{ color: 'var(--kit-text-primary)', fontWeight: 600 }}>{characterName || sessionCharacterId}</span>
            </div>
          )}
          {/* LoRA switcher chip */}
          <div style={{ position: 'relative' }}>
            ...
            >
              <span style={{ fontSize: 10, fontWeight: 700, color: 'var(--kit-text-tertiary)', textTransform: 'uppercase' }}>LoRA</span>
            ...
            {showLoraSwitcher && (
              <div style={{ position: 'absolute', top: '100%', left: 0, zIndex: 100, marginTop: 4, background: 'var(--kit-elevated)', border: '1px solid var(--kit-border)', borderRadius: 8, padding: 8, minWidth: 200, boxShadow: '0 8px 24px rgba(0,0,0,0.4)' }}>
                <button
                  className="kit-btn-secondary"
                ...
                {sessionLoraProfiles.map(p => {
                  ...
                  return (
                    <button
                      key={pid}
                      className="kit-btn-secondary"
                      style={{ display: 'block', width: '100%', textAlign: 'left', fontSize: 12, padding: '6px 10px', marginBottom: 2, fontWeight: isActive ? 700 : 400, color: isActive ? 'var(--kit-accent, #d97706)' : 'inherit' }}
                ...
                {sessionLoraProfiles.length === 0 && (
                  <p style={{ fontSize: 11, color: 'var(--kit-text-tertiary)', padding: '4px 10px', margin: 0 }}>No LoRA profiles for this character.</p>
                )}
              </div>
            )}
          </div>
      ...
      {/* Preset picker */}
      {showPresetPicker && (
        <div style={{ background: 'var(--kit-elevated)', border: '1px solid var(--kit-border)', borderRadius: 12, padding: 24, marginBottom: 16 }}>
          <h3 style={{ fontSize: 15, fontWeight: 600, marginBottom: 4 }}>Choose a Story Preset</h3>
          <p style={{ fontSize: 12, color: 'var(--kit-text-tertiary)', marginBottom: 16 }}>Select the narrative universe for your new session.</p>
            ...
            {presets.map(p => (
              <label
                ...
                style={{
                  ...
                  background: selectedPresetId === p.id ? 'var(--kit-accent-muted, rgba(217,119,6,0.1))' : 'transparent',
                  border: `1px solid ${selectedPresetId === p.id ? 'var(--kit-accent, #d97706)' : 'var(--kit-border)'}`,
              ...
              >
                ...
                <div>
                  ...
                  {p.synopsis && <div style={{ fontSize: 11, color: 'var(--kit-text-tertiary)', marginTop: 2 }}>{p.synopsis}</div>}
                  {p.protagonistName && <div style={{ fontSize: 11, color: 'var(--kit-text-secondary)', marginTop: 2 }}>Protagonist: {p.protagonistName}</div>}
                </div>
              </label>
            ))}
            {presets.length === 0 && (
              <p style={{ fontSize: 12, color: 'var(--kit-text-tertiary)' }}>No presets available.</p>
            )}
          </div>
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            {sessions.length > 0 && (
              <button className="kit-btn-secondary" onClick={() => setShowPresetPicker(false)}>
                Cancel
              </button>
            )}
          </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', marginBottom: 12 }}>
        {loadingSession ? (
          <div style={{ textAlign: 'center', padding: 32, color: 'var(--kit-text-tertiary)', fontSize: 13 }}>Loading session…</div>
        ) : (
          <div className="kit-chat-container">
            {messages.map((msg, i) => (
              <React.Fragment key={i}>
                ...
                {msg.role === 'system' ? (
                  <div style={{ textAlign: 'center', fontSize: 11, color: 'var(--kit-text-tertiary)', fontStyle: 'italic', margin: '6px 0', padding: '4px 12px', background: 'var(--kit-elevated)', borderRadius: 6, display: 'inline-block', width: '100%', boxSizing: 'border-box' }}>
                    {msg.text}
                  </div>
                ) : (
                <div className={`kit-chat-bubble ${msg.role}`}><ReactMarkdown>{msg.text}</ReactMarkdown></div>
                )}
                ...
                {msg.role === 'assistant' && !msg.sceneId && activeSessionId && (
                  <div style={{ marginBottom: 6 }}>
                    {illustratingMessages.has(i) ? (
                      <span style={{ fontSize: 11, color: 'var(--kit-text-tertiary)', fontStyle: 'italic' }}>Generating illustration…</span>
                    ) : (
                      <button
                        className="kit-btn-secondary"
            ...
            {sending && <div className="kit-chat-bubble assistant" style={{ opacity: 0.6 }}>Thinking…</div>}
            <div ref={bottomRef} />
          </div>
        )}
      </div>
