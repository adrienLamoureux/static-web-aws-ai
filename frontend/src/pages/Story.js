    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 112px)' }}>
      ...
      {activeSessionId && (sessionCharacterId || sessionLoraProfileId) && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10, flexWrap: 'wrap', position: 'relative' }}>
          {sessionCharacterId && (
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 5, background: 'var(--skr-elevated)', border: '1px solid var(--skr-border)', borderRadius: 6, padding: '3px 10px', fontSize: 12 }}>
              <span style={{ fontSize: 10, fontWeight: 700, color: 'var(--skr-text-tertiary)', textTransform: 'uppercase' }}>Character</span>
              <span style={{ color: 'var(--skr-text-primary)', fontWeight: 600 }}>{characterName || sessionCharacterId}</span>
            </div>
          )}
          {/* LoRA switcher chip */}
          <div style={{ position: 'relative' }}>
            ...
            >
              <span style={{ fontSize: 10, fontWeight: 700, color: 'var(--skr-text-tertiary)', textTransform: 'uppercase' }}>LoRA</span>
            ...
            {showLoraSwitcher && (
              <div style={{ position: 'absolute', top: '100%', left: 0, zIndex: 100, marginTop: 4, background: 'var(--skr-elevated)', border: '1px solid var(--skr-border)', borderRadius: 8, padding: 8, minWidth: 200, boxShadow: '0 8px 24px rgba(0,0,0,0.4)' }}>
                <button
                  className="skr-btn-secondary"
                ...
                {sessionLoraProfiles.map(p => {
                  ...
                  return (
                    <button
                      key={pid}
                      className="skr-btn-secondary"
                      style={{ display: 'block', width: '100%', textAlign: 'left', fontSize: 12, padding: '6px 10px', marginBottom: 2, fontWeight: isActive ? 700 : 400, color: isActive ? 'var(--skr-accent, #d97706)' : 'inherit' }}
                ...
                {sessionLoraProfiles.length === 0 && (
                  <p style={{ fontSize: 11, color: 'var(--skr-text-tertiary)', padding: '4px 10px', margin: 0 }}>No LoRA profiles for this character.</p>
                )}
              </div>
            )}
          </div>
      ...
      {showPresetPicker && (
        <div style={{ background: 'var(--skr-elevated)', border: '1px solid var(--skr-border)', borderRadius: 12, padding: 24, marginBottom: 16 }}>
            ...
            {presets.map(p => (
              <label
                ...
                style={{
                  ...
                  background: selectedPresetId === p.id ? 'var(--skr-accent-muted, rgba(217,119,6,0.1))' : 'transparent',
              ...
              >
                ...
                <div>
                  ...
                  {p.synopsis && <div style={{ fontSize: 11, color: 'var(--skr-text-tertiary)', marginTop: 2 }}>{p.synopsis}</div>}
                  {p.protagonistName && <div style={{ fontSize: 11, color: 'var(--skr-text-secondary)', marginTop: 2 }}>Protagonist: {p.protagonistName}</div>}
                </div>
              </label>
            ))}
            {presets.length === 0 && (
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            {sessions.length > 0 && (
              <button className="skr-btn-secondary" onClick={() => setShowPresetPicker(false)}>
                Cancel
              </button>
            )}
          </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', marginBottom: 12 }}>
        {loadingSession ? (
          <div style={{ textAlign: 'center', padding: 32, color: 'var(--skr-text-tertiary)', fontSize: 13 }}>Loading session…</div>
        ) : (
          <div className="skr-chat-container">
            {messages.map((msg, i) => (
              <React.Fragment key={i}>
                ...
                {msg.role === 'system' ? (
                  <div style={{ textAlign: 'center', fontSize: 11, color: 'var(--skr-text-tertiary)', fontStyle: 'italic', margin: '6px 0', padding: '4px 12px', background: 'var(--skr-elevated)', borderRadius: 6, display: 'inline-block', width: '100%', boxSizing: 'border-box' }}>
                    {msg.text}
                  </div>
                ) : (
                <div className={`skr-chat-bubble ${msg.role}`}><ReactMarkdown>{msg.text}</ReactMarkdown></div>
                )}
                ...
                {msg.role === 'assistant' && !msg.sceneId && activeSessionId && (
                  <div style={{ marginBottom: 6 }}>
                    {illustratingMessages.has(i) ? (
                      <span style={{ fontSize: 11, color: 'var(--skr-text-tertiary)', fontStyle: 'italic' }}>Generating illustration…</span>
                    ) : (
                      <button
                        className="skr-btn-secondary"
            ...
            {sending && <div className="skr-chat-bubble assistant" style={{ opacity: 0.6 }}>Thinking…</div>}
            <div ref={bottomRef} />
          </div>
        )}
      </div>
