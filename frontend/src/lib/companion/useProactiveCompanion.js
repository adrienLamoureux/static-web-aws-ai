...
export default function useProactiveCompanion() {
  ...
  const canFire = useCallback((trigger) => {
    ...
    if (
      triggerTimesRef.current[trigger] &&
      now - triggerTimesRef.current[trigger] < TRIGGER_COOLDOWN_MS
    )
      return false;
    return true;
  }, []);
  ...
  const fetchProactive = useCallback(
    async (trigger, payload) => {
      ...
      try {
        const data = await postJson(buildApiUrl(apiBaseUrl, COMPANION_PROACTIVE), {
          trigger,
          context,
        });
