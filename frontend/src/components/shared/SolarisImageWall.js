  ...
  return (
    <>
      ...
      >
        {showCta && (
          <div
            className="sol-image-tile sol-image-tile-cta"
            role="button"
            tabIndex={0}
            onClick={onCta}
            onKeyDown={(event) => {
          ...
          >
            <span className="sol-image-tile-plus">+</span>
          </div>
        )}
        {images.length > 0 ? (
          images.map((image, index) => (
            ...
            >
              <span className="sol-image-tile-media">
                <img
                  className="sol-image-tile-backdrop"
                  src={image.url}
                  alt=""
                  aria-hidden="true"
                />
                <img
                  className="sol-image-tile-main"
                  src={image.url}
                  alt={image.key || "Generated image"}
                />
              </span>
              <div className="sol-image-tile-overlay" />
              <span className="sol-image-tile-caption">{toDisplayLabel(image, index)}</span>
              <span className="sol-image-tile-actions">
                {onOpenVideo && (
                  <button
                    type="button"
                ...
                <button
                  type="button"
                  className={`sol-icon-btn${image.favorite ? " is-favorite" : ""}`}
                  onClick={(event) => {
                    event.stopPropagation();
          ...
          <div style={{ padding: '20px', color: 'var(--sol-text-tertiary)', fontSize: 13 }}>
            {status === "loading"
              ? "Loading images..."
              : "No images found yet."}
          </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '4px 0', fontSize: 11, color: 'var(--sol-text-tertiary)' }}>
        <span>
