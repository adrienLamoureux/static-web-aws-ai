  ...
  return (
    <>
      ...
      >
        {showCta && (
          <div
            className="yk-image-tile yk-image-tile-cta"
            role="button"
            tabIndex={0}
            onClick={onCta}
            onKeyDown={(event) => {
          ...
          >
            <span className="yk-image-tile-plus">+</span>
          </div>
        )}
        {images.length > 0 ? (
          images.map((image, index) => (
            ...
            >
              <span className="yk-image-tile-media">
                <img
                  className="yk-image-tile-backdrop"
                  src={image.url}
                  alt=""
                  aria-hidden="true"
                />
                <img
                  className="yk-image-tile-main"
                  src={image.url}
                  alt={image.key || "Generated image"}
                />
              </span>
              <div className="yk-image-tile-overlay" />
              <span className="yk-image-tile-caption">{toDisplayLabel(image, index)}</span>
              <span className="yk-image-tile-actions">
                {onOpenVideo && (
                  <button
                    type="button"
                ...
                <button
                  type="button"
                  className={`yk-icon-btn${image.favorite ? " is-favorite" : ""}`}
                  onClick={(event) => {
                    event.stopPropagation();
          ...
          <div style={{ padding: '20px', color: 'var(--yk-text-tertiary)', fontSize: 13 }}>
            {status === "loading"
              ? "Loading images..."
              : "No images found yet."}
          </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '4px 0', fontSize: 11, color: 'var(--yk-text-tertiary)' }}>
        <span>
