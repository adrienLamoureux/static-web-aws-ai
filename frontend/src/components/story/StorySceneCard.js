  ...
  return (
    <div style={{
      ...
      <div style={{ padding: '8px 12px', display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
        {scene.title && (
          <span style={{ fontSize: 12, color: 'var(--sol-text-secondary)', flex: 1, fontStyle: 'italic', minWidth: 80 }}>
            {scene.title}
          </span>
        )}
      ...
      {(animStatusLabel || musicStatusLabel) && (
        <div style={{ padding: '0 12px 6px', display: 'flex', gap: 12 }}>
          {animStatusLabel && (
            <span style={{ fontSize: 10, color: 'var(--sol-text-tertiary)' }}>{animStatusLabel}</span>
          )}
          {musicStatusLabel && (
            <span style={{ fontSize: 10, color: 'var(--sol-text-tertiary)' }}>{musicStatusLabel}</span>
          )}
        </div>
      )}
