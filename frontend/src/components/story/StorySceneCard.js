  ...
  return (
    <div style={{
      ...
      <div style={{ padding: '8px 12px', display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
        {scene.title && (
          <span style={{ fontSize: 12, color: 'var(--kit-text-secondary)', flex: 1, fontStyle: 'italic', minWidth: 80 }}>
            {scene.title}
          </span>
        )}
        ...
        {scene.illustrating && (
          <span style={{ fontSize: 11, color: 'var(--kit-text-tertiary)' }}>Generating…</span>
        )}
        {hasImage && (
          <button
            className="kit-btn-secondary"
      ...
      {(animStatusLabel || musicStatusLabel) && (
        <div style={{ padding: '0 12px 6px', display: 'flex', gap: 12 }}>
          {animStatusLabel && (
            <span style={{ fontSize: 10, color: 'var(--kit-text-tertiary)' }}>{animStatusLabel}</span>
          )}
          {musicStatusLabel && (
            <span style={{ fontSize: 10, color: 'var(--kit-text-tertiary)' }}>{musicStatusLabel}</span>
          )}
        </div>
      )}
