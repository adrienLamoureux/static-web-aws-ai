...
function SolarisMusicDock({ nowPlayingTrack: nowPlayingProp, availableTracks: availProp }) {
  ...
  const barsMarkup = (
    <div
      className={`yk-music-bars${isPeak ? " is-peak" : ""}${
    ...
    >
      {bars.map((value, index) => (
        <span
          ...
          className="yk-music-bar"
          style={{ "--yk-bar-scale": clamp(value, 0, 1), height: `${clamp(value, 0, 1) * 100}%` }}
        />
      ))}
    </div>
  );
  ...
  return (
    <aside
      className={`yk-music-dock${hasTrack ? "" : " is-idle"}${
    ...
    >
      ...
      {isExpanded && (
        <div className="yk-music-expanded-panel">
          ...
          {(sourceLabel || selectedTrack?.mood || selectedTrack?.energy || tagList.length > 0) && (
            <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', marginTop: 6 }}>
              {sourceLabel ? <span className="yk-lora-chip">{sourceLabel}</span> : null}
              {selectedTrack?.mood ? <span className="yk-lora-chip">{selectedTrack.mood}</span> : null}
              {selectedTrack?.energy ? <span className="yk-lora-chip">{selectedTrack.energy}</span> : null}
              {tagList.map((tag, index) => (
                <span key={`${trackKey || "track"}-${tag}-${index}`} className="yk-track-tag">{tag}</span>
              ))}
            </div>
          )}
        </div>
      ...
      <div className="yk-music-compact-bar">
        {barsMarkup}

        <button
          type="button"
          className="yk-music-controls-btn"
        ...
        {hasTrack && (
          <>
            <span className="yk-music-time">{formatTime(elapsedSeconds)}</span>
            <input
              type="range"
              className="yk-music-progress"
            ...
            />
            <span className="yk-music-time">{formatTime(durationSeconds)}</span>
          </>
        )}

        <button
          type="button"
          className={`yk-music-controls-btn${isLooping ? " active" : ""}`}
        ...
        <button
          type="button"
          className={`yk-music-controls-btn${isMuted ? " active" : ""}`}
