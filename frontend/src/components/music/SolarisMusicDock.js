...
function SolarisMusicDock({ nowPlayingTrack: nowPlayingProp, availableTracks: availProp }) {
  ...
  const barsMarkup = (
    <div
      className={`kit-music-bars${isPeak ? " is-peak" : ""}${
    ...
    >
      {bars.map((value, index) => (
        <span
          ...
          className="kit-music-bar"
          style={{ "--kit-bar-scale": clamp(value, 0, 1), height: `${clamp(value, 0, 1) * 100}%` }}
        />
      ))}
    </div>
  );
  ...
  return (
    <aside
      className={`kit-music-dock${hasTrack ? "" : " is-idle"}${
    ...
    >
      ...
      {isExpanded && (
        <div className="kit-music-expanded-panel">
          ...
          {(sourceLabel || selectedTrack?.mood || selectedTrack?.energy || tagList.length > 0) && (
            <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', marginTop: 6 }}>
              {sourceLabel ? <span className="kit-lora-chip">{sourceLabel}</span> : null}
              {selectedTrack?.mood ? <span className="kit-lora-chip">{selectedTrack.mood}</span> : null}
              {selectedTrack?.energy ? <span className="kit-lora-chip">{selectedTrack.energy}</span> : null}
              {tagList.map((tag, index) => (
                <span key={`${trackKey || "track"}-${tag}-${index}`} className="kit-track-tag">{tag}</span>
              ))}
            </div>
          )}
        </div>
      ...
      <div className="kit-music-compact-bar">
        {barsMarkup}

        <button
          type="button"
          className="kit-music-controls-btn"
        ...
        <span
          className="kit-music-track-name"
        ...
        {hasTrack && (
          <>
            <span className="kit-music-time">{formatTime(elapsedSeconds)}</span>
            <input
              type="range"
              className="kit-music-progress"
            ...
            />
            <span className="kit-music-time">{formatTime(durationSeconds)}</span>
          </>
        )}

        <button
          type="button"
          className={`kit-music-controls-btn${isLooping ? " active" : ""}`}
        ...
        <button
          type="button"
          className={`kit-music-controls-btn${isMuted ? " active" : ""}`}
