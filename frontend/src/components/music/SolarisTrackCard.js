...
function SolarisTrackCard({
  ...
  return (
    <div
      className={`skr-track-item ${isSelected ? "skr-track-item--selected" : ""}${
        isStoryV3Variant ? " skr-track-card" : ""
      }`}
    >
      {/* Title row */}
      ...
      {showLoopToggle && loop && (
        <p className="skr-track-loop-note">
          {prefersNativeLoop
      ...
      {showMeta && (
        <div style={{ display: "flex", flexWrap: "wrap", gap: 4, marginTop: 6 }}>
          ...
          {tempoBpm ? <span className="skr-track-pill tempo">Tempo: {tempoBpm}</span> : null}
        </div>
      )}
