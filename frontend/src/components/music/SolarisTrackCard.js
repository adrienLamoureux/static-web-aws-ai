...
function SolarisTrackCard({
  ...
  return (
    <div
      className={`kit-track-item ${isSelected ? "kit-track-item--selected" : ""}${
        isStoryV3Variant ? " kit-track-card" : ""
      }`}
    >
      {/* Title row */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <p className="kit-track-title">{title || "Untitled track"}</p>
          {stampLabel && <span style={{ fontSize: 10, color: 'var(--kit-text-tertiary)' }}>{stampLabel}</span>}
        </div>
      ...
      {description && <p style={{ fontSize: 12, color: 'var(--kit-text-secondary)', marginTop: 4 }}>{description}</p>}

      {/* Full-width audio player */}
      {url && (
        <audio
          ref={audioRef}
          className="kit-track-audio"
          controls
          preload={preloadMode}
          src={url}
      ...
      {showLoopToggle && loop && (
        <p className="kit-track-loop-note">
          {prefersNativeLoop
      ...
      {showMeta && (
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginTop: 6 }}>
          {sourceLabel && <span className="kit-track-pill">{sourceLabel}</span>}
          {mood && <span className="kit-track-pill mood">Mood: {mood}</span>}
          {energy && <span className="kit-track-pill energy">Energy: {energy}</span>}
          {tempoBpm ? <span className="kit-track-pill tempo">Tempo: {tempoBpm}</span> : null}
        </div>
      )}
      ...
      {safeTagList.length > 0 && (
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 3, marginTop: 4 }}>
          {safeTagList.map((tag, index) => (
            <span key={`${trackId || key || title || "track"}-${tag}-${index}`} className="kit-track-tag">
              {tag}
            </span>
          ))}
        </div>
...
export default SolarisTrackCard;
