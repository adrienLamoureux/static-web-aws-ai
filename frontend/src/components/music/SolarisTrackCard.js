...
function SolarisTrackCard({
  ...
  return (
    <div
      className={`sol-track-item ${isSelected ? "sol-track-item--selected" : ""}${
    ...
    >
      ...
      {showMeta && (
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginTop: 6 }}>
          ...
          {mood && <span className="sol-track-pill mood">Mood: {mood}</span>}
          {energy && <span className="sol-track-pill energy">Energy: {energy}</span>}
      ...
      {safeTagList.length > 0 && (
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 3, marginTop: 4 }}>
          {safeTagList.map((tag, index) => (
            <span key={`${trackId || key || title || "track"}-${tag}-${index}`} className="sol-track-tag">
              {tag}
            </span>
          ))}
        </div>
