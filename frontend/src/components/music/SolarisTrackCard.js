...
function SolarisTrackCard({
  ...
  return (
    <div
      className={`yk-track-item ${isSelected ? "yk-track-item--selected" : ""}${
        isStoryV3Variant ? " yk-track-card" : ""
      }`}
    >
      {/* Title row */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <p className="yk-track-title">{title || "Untitled track"}</p>
          {stampLabel && <span style={{ fontSize: 10, color: 'var(--yk-text-tertiary)' }}>{stampLabel}</span>}
        </div>
      ...
      {description && <p style={{ fontSize: 12, color: 'var(--yk-text-secondary)', marginTop: 4 }}>{description}</p>}

      {/* Full-width audio player */}
      {url && (
        <audio
          ref={audioRef}
          className="yk-track-audio"
          controls
          preload={preloadMode}
          src={url}
      ...
      {showLoopToggle && loop && (
        <p className="yk-track-loop-note">
          {prefersNativeLoop
      ...
      {showMeta && (
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginTop: 6 }}>
          {sourceLabel && <span className="yk-track-pill">{sourceLabel}</span>}
          {mood && <span className="yk-track-pill mood">Mood: {mood}</span>}
          {energy && <span className="yk-track-pill energy">Energy: {energy}</span>}
          {tempoBpm ? <span className="yk-track-pill tempo">Tempo: {tempoBpm}</span> : null}
        </div>
      )}
      ...
      {safeTagList.length > 0 && (
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 3, marginTop: 4 }}>
          {safeTagList.map((tag, index) => (
            <span key={`${trackId || key || title || "track"}-${tag}-${index}`} className="yk-track-tag">
              {tag}
            </span>
          ))}
        </div>
