...
export default function SolarisMasonry({ images = [], title, subtitle }) {
  ...
  return (
    <div className="sol-masonry-hero" aria-hidden="true">
      <div className="sol-masonry-grid">
        {COLUMNS.map((col) => (
          <div
            key={col.id}
            className="sol-masonry-column"
            style={{
              "--sol-masonry-duration": `${col.durationSeconds}s`,
              "--sol-masonry-start": col.startOffset,
            }}
          >
            {looped.map((img) => (
              <figure key={`${col.id}-${img.loopId}`} className="sol-masonry-card">
      ...
      {(title || subtitle) && (
        <div className="sol-masonry-overlay">
          {title   && <p className="sol-masonry-title">{title}</p>}
          {subtitle && <p className="sol-masonry-subtitle">{subtitle}</p>}
        </div>
      )}
    </div>
  );
