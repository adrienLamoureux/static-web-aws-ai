...
export default function SolarisMasonry({ images = [], title, subtitle }) {
  ...
  return (
    <div className="kit-masonry" aria-hidden="true">
      <div className="kit-masonry-grid">
        {COLUMNS.map((col) => (
          <div
            key={col.id}
            className="kit-masonry-column"
            style={{
              "--kit-masonry-duration": `${col.durationSeconds}s`,
              "--kit-masonry-start": col.startOffset,
            }}
          >
            {looped.map((img) => (
              <figure key={`${col.id}-${img.loopId}`} className="kit-masonry-card">
      ...
      {(title || subtitle) && (
        <div className="kit-masonry-overlay">
          {title   && <p className="kit-masonry-title">{title}</p>}
          {subtitle && <p className="kit-masonry-subtitle">{subtitle}</p>}
        </div>
      )}
    </div>
  );
