...
describe('fetchJson', () => {
  ...
  it('returns parsed JSON on a 200 response', async () => {
    ...
    jest.spyOn(global, 'fetch').mockReturnValue(
      makeFetchResponse({ status: 200, ok: true, body: data })
    );
  ...
  it('uses fallback error message when body has no message', async () => {
    jest.spyOn(global, 'fetch').mockReturnValue(
      makeFetchResponse({ status: 500, ok: false, body: {} })
    );
...
describe('postJson', () => {
  ...
  it('sends a POST with JSON body and Content-Type header', async () => {
    ...
    const fetchSpy = jest.spyOn(global, 'fetch').mockReturnValue(
      makeFetchResponse({ status: 200, ok: true, body: responseData })
    );
  ...
  it('includes Authorization header when token is set', async () => {
    ...
    const fetchSpy = jest.spyOn(global, 'fetch').mockReturnValue(
      makeFetchResponse({ status: 200, ok: true, body: {} })
    );
...
describe('deleteJson', () => {
  ...
  it('throws on a failed DELETE request', async () => {
    ...
    await expect(deleteJson('https://example.com/api/resource/999')).rejects.toThrow(
      'Not Found'
    );
  });
});
