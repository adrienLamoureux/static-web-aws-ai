# Runbook - design-sakura

## Context
- Idea ID: `design-sakura`
- Stack ID: `StaticWebAWSAIStack-design-sakura`
- Idea directory: `ideas/design-sakura`

## One-Time Setup
1. Ensure dependencies are installed for `frontend/` and `cdk/`.
2. Initialize context docs:
`npm --prefix cdk run idea:init -- --stage=design-sakura --title="Sakura Bloom — Maximalist Immersive Anime UI"`
...
`npm --prefix cdk run idea:deploy -- --stage=design-sakura`
2. Optional deploy without rebuilding frontend:
`npm --prefix cdk run idea:deploy -- --stage=design-sakura --skip-build`
3. Optional ownership/lifecycle tags:
`npm --prefix cdk run idea:deploy -- --stage=design-sakura --owner="adrien" --ttl-days=14`

## Seed Default Content
1. Seed this idea from the baseline source stage (`sandbox` by default):
`npm --prefix cdk run idea:seed -- --target-stage=design-sakura --source-stage=sandbox`
2. Seed this idea from an explicit source stack name:
`npm --prefix cdk run idea:seed -- --target-stage=design-sakura --source-stack=StaticWebAWSAIStack`
3. Seed with explicit user credentials:
`npm --prefix cdk run idea:seed -- --target-stage=design-sakura --source-stage=sandbox --seed-user-email="seed+design-sakura@example.com" --seed-user-password="<strong-password>"`
...
`npm --prefix cdk run idea:deploy-many -- --stages=design-sakura,other-idea --improvement="improvement-name"`
...
## Inspect
1. Synthesize:
`npm --prefix cdk run idea:synth -- --stage=design-sakura`
2. Diff:
`npm --prefix cdk run idea:diff -- --stage=design-sakura`
