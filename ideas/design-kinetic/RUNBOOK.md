# Runbook - design-kinetic

## Context
- Idea ID: `design-kinetic`
- Stack ID: `StaticWebAWSAIStack-design-kinetic`
- Idea directory: `ideas/design-kinetic`

## One-Time Setup
1. Ensure dependencies are installed for `frontend/` and `cdk/`.
2. Initialize context docs:
`npm --prefix cdk run idea:init -- --stage=design-kinetic --title="Kinetic — dark spatial canvas"`
...
`npm --prefix cdk run idea:deploy -- --stage=design-kinetic`
2. Optional deploy without rebuilding frontend:
`npm --prefix cdk run idea:deploy -- --stage=design-kinetic --skip-build`
3. Optional ownership/lifecycle tags:
`npm --prefix cdk run idea:deploy -- --stage=design-kinetic --owner="adrien" --ttl-days=14`

## Seed Default Content
1. Seed this idea from the baseline source stage (`sandbox` by default):
`npm --prefix cdk run idea:seed -- --target-stage=design-kinetic --source-stage=sandbox`
2. Seed this idea from an explicit source stack name:
`npm --prefix cdk run idea:seed -- --target-stage=design-kinetic --source-stack=StaticWebAWSAIStack`
3. Seed with explicit user credentials:
`npm --prefix cdk run idea:seed -- --target-stage=design-kinetic --source-stage=sandbox --seed-user-email="seed+design-kinetic@example.com" --seed-user-password="<strong-password>"`
...
`npm --prefix cdk run idea:deploy-many -- --stages=design-kinetic,other-idea --improvement="improvement-name"`
...
## Inspect
1. Synthesize:
`npm --prefix cdk run idea:synth -- --stage=design-kinetic`
2. Diff:
`npm --prefix cdk run idea:diff -- --stage=design-kinetic`

## Destroy
1. Destroy full stack:
`npm --prefix cdk run idea:destroy -- --stage=design-kinetic`
