# Runbook - design-solaris

## Context
- Idea ID: `design-solaris`
- Stack ID: `StaticWebAWSAIStack-design-solaris`
- Idea directory: `ideas/design-solaris`

## One-Time Setup
1. Ensure dependencies are installed for `frontend/` and `cdk/`.
2. Initialize context docs:
`npm --prefix cdk run idea:init -- --stage=design-solaris --title="Solaris — warm light-first creative workspace"`
...
`npm --prefix cdk run idea:deploy -- --stage=design-solaris`
2. Optional deploy without rebuilding frontend:
`npm --prefix cdk run idea:deploy -- --stage=design-solaris --skip-build`
3. Optional ownership/lifecycle tags:
`npm --prefix cdk run idea:deploy -- --stage=design-solaris --owner="adrien" --ttl-days=14`

## Seed Default Content
1. Seed this idea from the baseline source stage (`sandbox` by default):
`npm --prefix cdk run idea:seed -- --target-stage=design-solaris --source-stage=sandbox`
2. Seed this idea from an explicit source stack name:
`npm --prefix cdk run idea:seed -- --target-stage=design-solaris --source-stack=StaticWebAWSAIStack`
3. Seed with explicit user credentials:
`npm --prefix cdk run idea:seed -- --target-stage=design-solaris --source-stage=sandbox --seed-user-email="seed+design-solaris@example.com" --seed-user-password="<strong-password>"`
...
`npm --prefix cdk run idea:deploy-many -- --stages=design-solaris,other-idea --improvement="improvement-name"`
...
## Inspect
1. Synthesize:
`npm --prefix cdk run idea:synth -- --stage=design-solaris`
2. Diff:
`npm --prefix cdk run idea:diff -- --stage=design-solaris`

## Destroy
1. Destroy full stack:
`npm --prefix cdk run idea:destroy -- --stage=design-solaris`
