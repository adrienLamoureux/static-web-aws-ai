# Runbook - design-moescape

## Context
- Idea ID: `design-moescape`
- Stack ID: `StaticWebAWSAIStack-design-moescape`
- Idea directory: `ideas/design-moescape`

## One-Time Setup
1. Ensure dependencies are installed for `frontend/` and `cdk/`.
2. Initialize context docs:
`npm --prefix cdk run idea:init -- --stage=design-moescape --title="Moescape inspired"`
...
`npm --prefix cdk run idea:deploy -- --stage=design-moescape`
2. Optional deploy without rebuilding frontend:
`npm --prefix cdk run idea:deploy -- --stage=design-moescape --skip-build`
3. Optional ownership/lifecycle tags:
`npm --prefix cdk run idea:deploy -- --stage=design-moescape --owner="adrien" --ttl-days=14`

## Seed Default Content
1. Seed this idea from the baseline source stack:
`npm --prefix cdk run idea:seed -- --target-stage=design-moescape --source-stack=StaticWebAWSAIStack`
2. Seed with explicit user credentials:
`npm --prefix cdk run idea:seed -- --target-stage=design-moescape --source-stack=StaticWebAWSAIStack --seed-user-email="seed+design-moescape@example.com" --seed-user-password="<strong-password>"`
...
`npm --prefix cdk run idea:deploy-many -- --stages=design-moescape,other-idea --improvement="improvement-name"`
...
## Inspect
1. Synthesize:
`npm --prefix cdk run idea:synth -- --stage=design-moescape`
2. Diff:
`npm --prefix cdk run idea:diff -- --stage=design-moescape`

## Destroy
1. Destroy full stack:
`npm --prefix cdk run idea:destroy -- --stage=design-moescape`
