# Runbook - design-endfield

## Context
- Idea ID: `design-endfield`
- Stack ID: `StaticWebAWSAIStack-design-endfield`
- Idea directory: `ideas/design-endfield`

## One-Time Setup
1. Ensure dependencies are installed for `frontend/` and `cdk/`.
2. Initialize context docs:
`npm --prefix cdk run idea:init -- --stage=design-endfield --title="Arknights Endfield inspired"`
...
`npm --prefix cdk run idea:deploy -- --stage=design-endfield`
2. Optional deploy without rebuilding frontend:
`npm --prefix cdk run idea:deploy -- --stage=design-endfield --skip-build`
3. Optional ownership/lifecycle tags:
`npm --prefix cdk run idea:deploy -- --stage=design-endfield --owner="adrien" --ttl-days=14`

## Seed Default Content
1. Seed this idea from the baseline source stack:
`npm --prefix cdk run idea:seed -- --target-stage=design-endfield --source-stack=StaticWebAWSAIStack`
2. Seed with explicit user credentials:
`npm --prefix cdk run idea:seed -- --target-stage=design-endfield --source-stack=StaticWebAWSAIStack --seed-user-email="seed+design-endfield@example.com" --seed-user-password="<strong-password>"`
...
`npm --prefix cdk run idea:deploy-many -- --stages=design-endfield,other-idea --improvement="improvement-name"`
...
## Inspect
1. Synthesize:
`npm --prefix cdk run idea:synth -- --stage=design-endfield`
2. Diff:
`npm --prefix cdk run idea:diff -- --stage=design-endfield`

## Destroy
1. Destroy full stack:
`npm --prefix cdk run idea:destroy -- --stage=design-endfield`
